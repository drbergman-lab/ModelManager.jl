...
### Trigger
Issue #52, from the 0.9 architecture review. The 0.9 seam unified a measurement's *input* side — every
consumer calls `compute(::Simulation)` and reduces through one `_reduceOverMonad` — but the *value*
side was three contracts wearing one struct: the sink took a `String`, GSA demanded a `Real` or a
keyed value of them, calibration took anything its `distance` did. So
`QoI("mse", counts; reduce = discrepancy)` stored raw per-simulation counts as `mse.tumor` in the sink
while GSA reported one scalar `mse`. Three passes; only the third is live, the other two under
Rejected.

### Decisions
- **The seam interprets nothing; each consumer applies the rule it needs.** `compute`'s value has one
  reader (the sink) and `reduce`'s has two others (GSA, calibration), so one rule over both
  constrained things that share no reader and refused values before the function written to handle
  them ran. `_qoiValueShape` (a `Real`, or a flat non-empty `Dict`/`NamedTuple` of `Real`s) is called
  by `_postProcess` and `evaluateFunctionOnSampling`; calibration requires nothing. Errors name
  *whose* rule was broken: a user told "a value must be a `Real`" without that qualification rewrites
  a `compute` their `distance` would have taken unchanged.
- **`reduce` need not return the shape it was given**, and replicate key agreement is the DEFAULT
  reducer's rule, enforced in `_qoiMean`. Enforcing it earlier meant a reducer written to reconcile
  ragged replicates never saw them — PCMM's `endpointPopulationCountQoI` zero-fills exactly that and
  was unreachable. `_qoiMean` must not require `Real` components either, `Dict(key => Vector)`
  averaging elementwise being PCMM's time-series shape.
- **`SummaryValues` is one key space for all three consumers, and holds `Any`.** A component is
  `(qoi name, component key)`, `nothing` for a `Real`-valued QoI; a `String` lookup resolves in one
  order — the scalar name, a split at the FIRST `.`, then a bare component key when exactly one QoI
  reports it. The `Any` is load-bearing: a `Float64` element type forced every `reduce` to produce
  numbers before `distance`, the reader that decides, had seen anything. `_evaluateSummary` always
  returns one, so `q` and `[q]` are one contract.
- **`missing` says "no value here"; `nothing` is refused** on both paths, `nothing` being what a block
  that falls through returns. Breaking for the sink: a side-effects-only `post_processor` must end
  with `missing`. A `missing` summary follows `on_monad_failure`, and the check sits *between* the
  summary and distance calls so a `missing` from the user's own `distance` still raises "a `Real` is
  required". Such a particle gets its own per-generation `@warn`, nothing having failed on that path
  and no failure file being written, which made a measurement that never has a value look exactly
  like a model that never fits.
- **One naming rule, two spreaders.** `_qoiComponentLabels` holds the naming and the `1`-vs-`"1"`
  collision refusal; `_keyedEntries` is the strict `Float64` spreader for the sink and GSA,
  `_summaryEntries` calibration's permissive one. An empty keyed value contributes zero entries there
  rather than landing whole under `(name, nothing)`, which would make the summary's key space depend
  on the value.
- **QoI names need not be unique; the `(name, key)` pair must be.** Keyed QoIs sharing a name with
  disjoint keys is ordinary, so each consumer checks its own collision — the `_SummaryKey`, the sink
  column, the GSA label — instead of checking names.
- **`mseDistance` computes over the OBSERVED keys**; extra simulated components are ignored, since we
  always know more about a simulation than about the data. Three guards replace the dropped
  strictness: an observation naming nothing is refused (0/0 would make every particle perfect and
  hand ABC-SMC back the prior), two spellings resolving to one entry are refused naming both, and a
  resolved `missing` is refused naming its label. Normalisation is one global mean over the number of
  *differences*, so a one-key array summary is exactly `mean(abs2, sim .- obs)` and a 100-point series
  contributes 100 terms where a scalar key contributes one.
- **A `QoI` may carry its own `data`**, switching `compute`/`reduce` to two-argument form by an
  explicit rule rather than method sniffing, which would call a `compute` that merely happens to take
  two arguments with data it was never written for. It serialises inside the `QoI` in `problem.jld2`,
  so a resume needs nothing re-supplied and `_ProblemManifest` gained no field.
- **A validator plus a function, not a returned closure.** `_validatePostProcessor` runs once in `run`
  and `_postProcess(qs, sim)` per simulation, which lets both keywords be typed and is why `qoi.jl`
  is included **before** `runner.jl`: a signature is evaluated when the method is defined. GSA
  likewise wraps its reduction loop the way `_evaluateParticle` wraps user code, since without the
  seam's checks ahead of it a `compute` the default reducer cannot average throws from inside
  `_qoiMean`, which knows neither QoI nor monad.
- **Cleared the 0.9 transitional apparatus**: `_declaresSimulation`, `_WARNED_SUMMARIES`, the
  migration warning, `qoiName(::Function)` and `_computeOn(::QoI, ::Integer)`.

### Rejected
- **One value rule at the seam for every consumer** (passes one and two) — constrained values that
  share no reader, and made a struct-returning `compute` whose `reduce` yields a keyed series
  unevaluable although calibrating that series is its purpose.
- **`reduce` must preserve `compute`'s shape** (passes one and two) — the workaround it forced
  produced the very "same name, two quantities" the rule existed to prevent: a score carried as a key
  is a per-simulation score at the sink and a score-of-means in GSA. Survives as advice, not a rule.
- **Calibration keyed by the user's bare component names** (pass one) — made a QoI narrower used for
  calibration than for the sink or GSA, and forbade two keyed QoIs sharing a component key.
- **A strict-both-ways `mseDistance`** (pass two) — forced a user to list every component of every
  summary QoI in `observed_data` or drop QoIs from the summary.
- **Keeping `String` as a sink-only value** — tags are the queryable, retroactive home for text about
  a simulation, and dropping it drops the sink's TEXT branch.
- **`Vector`-valued QoIs**, for now; may well come back. Keys make the alignment explicit, the easier
  thing to build on first. What a vector needs settled: how it is labelled at the sink and in GSA, how
  it is matched across monads, how it is named in an `observed_data`. The earlier framing — that a
  vector can only be reasoned about by its length — is wrong and is gone from the docs and the error.
- **A more general `AbstractQoI` seam** whose `reduce` may return a matrix or a spread. Set aside for
  v0.10: worth designing against a concrete need rather than ahead of one.

### Traps
- **The `QoI` struct gained a field, and the two shapes of `summary_statistic` fail differently.** A
  single-`QoI` `problem.jld2` loads with a JLD2 reconstruction warning and resumes through `problem=`;
  a `Vector{QoI}` one **cannot be read at all**, JLD2 failing to convert the array's element so the
  whole `f["manifest"]` read throws before any `problem=` is consulted. That affects **released
  v0.9.1** files, and the vector form is the suite's canonical shape. The rescue is PR #56's
  `_loadProblem(; required=)`.
- **Errors raised inside `_postProcess` are per-simulation stage errors** (`_SimulationStageError`),
  not the bare `ArgumentError` the sink's own writer raises from the serial completion loop.
- **A QoI-written sink column is now REAL** whatever `compute` returned, `_keyedEntries` producing
  `Float64`; `_postProcessingColumnSpec`'s `Bool`/`Integer` branches stay for another route.
- **`filter(!ismissing, v)` would not narrow the element type**; `collect(skipmissing(v))` does.
- **`_summaryEntries` and `_keyedEntries` must not drift** — they agree only because both go through
  `_qoiComponentLabels`, and the key-ordering rules are spelled out in both, `_summaryEntries` being
  unable to call `_qoiValueShape`.
- **An empty keyed value behaves differently in the three consumers, deliberately**: refused by the
  sink and GSA, zero entries for calibration. A summary that names nothing is a fact `distance` can be
  told about; a column or an index cannot be built from one.

### Effect on PCMM (recorded for #232)
`meanPopulationTimeSeriesQoI` becomes usable in calibration with `mseDistance`, and
`endpointPopulationCountQoI`'s zero-filling reducer becomes reachable, turning a ragged monad's error
into a value. Pre-existing and separate: `populationCountQoI` returns `nothing` for a pruned snapshot,
which the sink refuses.

---

## Session: the MM/PCMM boundary — the half that needs no downstream companion (2026-09-06) — ships in v0.10.0

Issue #54, the architecture review of the ModelManager/PCMM split, restricted to the items needing no
companion change downstream. The four that *do* change the interface — collapsing the five
version-bookkeeping methods, the XML seam, an MM-owned schema version, and the `skip_missing` reducer
default — are deliberately not started.
...
- **`variationFilePath(location, M)` is a computation, not a lookup.** PCMM rebuilt
  ...
  which is what `createXMLFile` itself needs. `createXMLFile` now calls it, so the two cannot drift.
- **The unexported names stay unexported; the manual was the bug.** A downstream
  `@reexport using ModelManager` forwards only exports, so a name documented here as a bare call is an
  `UndefVarError` for that package's users. Dividing line: a name introduced in the third person as
  "the backend's `setupMonad` hook" stays bare; a name in a list of helpers, or in a code block, is
  written `ModelManager.name`. That caught five the issue's list missed.
- **`upgradeToMilestone`'s default throws rather than no-opping.** `upgradeMilestones` defaulting to
  empty makes it unreachable for a fresh backend, so the only way there is to have declared a
  milestone and not written the handler — silently skipping that would stamp the version table as
  migrated. `dbVersionTableName` derives from the package via #30's `_packageModule`, keeping one
  answer to "which package does this database track".
- **`registerSimulator!` is idempotent by backend *type***, so a package reload does not discard an
  open project's data directory, DB handle, or provenance (PR #65). `mm_globals_ref` is internal, not
  merely unexported: its only outside reader was a backend's `__init__` assigning to it, which
  `registerSimulator!` replaces.

### Rejected
- **Exporting issue #54's item-4 list** — dev contracts, not end-user API; export would put
  `simulatorVersionIDName` and friends in every user's tab completion to fix a docs problem.
- **Qualifying every `@compat public` mention in `docs/src/man/`** — misleading in sentences that
  already say "the backend's … hook" for names a reader is meant to *implement*.

### Traps
- **A default whose signature is more specific than an existing override is an ambiguity, not an
  override**: `upgradeToMilestone(sim::AbstractSimulator, version, args...)` against the suite's
  `(::TestSimulator, args...)`. Keep the default at the shape of the error stub it replaces.
- **`@compat public` on an already-exported name is a load-time error**, so a name must leave both
  `export` lists in the same change that declares it public.
- **The docstring-ref testset does not read manual pages** — it walks `Docs.meta` only, so an `@ref` in
  `docs/src/man/*.md` is caught by nothing but the docs build.

---

## Session: two holes found by a second review pass (2026-09-06)

- **The busy timeout covered the central database only.** The per-folder variations database created in
  `insertFolder` and the connection `locationVariationsDatabase` hands back bypassed `_openDB`, whose
  docstring claims every open goes through it, so a second session still failed at once with "database
  is locked" — precisely the two-session case the timeout was added for. Both go through `_openDB` now,
  and a test asserts `SQLite.DB(` appears exactly once in `database.jl` and nowhere else in `src/`, so
  the docstring stays true by construction rather than by inspection.
- **A deleted calibration left tags that were re-attributed, not merely stale.** Each generation's
  batch sampling carries `mm:calibration => "<id>"`, which `deleteTagsFor(Calibration, ids)` cannot
  reach: those rows sit on *sampling* rows and name the run in their **value**, while that function is
  keyed on `(trial_class, trial_id)`. With no `AUTOINCREMENT` on `calibration_id` SQLite hands the
  deleted run's id to the next one, so `findMonads(tags = ("mm:calibration" => …))` — the route `tag!`'s
  own docstring recommends — returned both runs' monads while `monadIDs(result)` disagreed. Decision:
  clean the value-references in `deleteCalibration` rather than add `AUTOINCREMENT`, whose schema change
  every backend would have to implement to fix a symptom.
- **Trap for the test:** `mm:generation` values are generation numbers, so *every* run in a project has
  a sampling tagged `"1"`. Assert on the samplings the run under test tagged.

---

## Session: `deleteCalibration(delete_subs=true)` keeps monads other runs use (2026-09-06) — ships in v0.10.0

Issue #58: `deleteSampling(delete_subs=true)` protects the monads other samplings list, while
`deleteCalibration(delete_subs=true)` handed its whole evaluated set to `deleteMonad` with no
cross-reference check. The `SimulationBank` is keyed on inputs and simulator version, not on the
calibration, so two runs on one problem *routinely* share monads.

### Decisions
- **`delete_subs=true` now means "the monads only this run used"**, the sibling function's semantics
  winning over keeping the cascade and documenting it; the default stays `false`.
- **The run's own batch samplings are excluded from the guard, by tag.** A naive mirror of
  `deleteSampling` protects *everything*, each generation's batch sampling listing that generation's
  monads; `_batchSamplingIDs` reads the `mm:calibration` tag `_buildEvaluateBatch` stamps, and nothing
  else can make the distinction, the generation records naming monads and never samplings. Other
  calibrations are guarded through `calibrationMonadIDs`, the raw on-disk record.
- **`delete_supers` stays `true`, against the issue's plan.** `deleteSampling` can pass `false` only
  because it has already removed its own rows; here the batch samplings stay in the table, and with
  `false` they kept constituent lists naming deleted monads, on which `Sampling(id)` threw.

### Rejected
- **`findTrials(Sampling; tags=...)` for the batch lookup** — it materializes `Sampling` objects and
  throws above `MAX_MATERIALIZED_TRIALS`, turning a long run into a deletion failure.

### Traps
- **Everything is read before anything is deleted** — the generation records live inside the folder
  `rm_hpc_safe` removes, and the `mm:calibration` tags are one extension away from being deleted with
  the calibration's own tag rows (#60).
- **A green suite did not catch the stale batch rows**, nothing having constructed a batch `Sampling`
  after a deletion; the regression test now materialises each surviving batch.
- **A test here proves nothing unless the two runs really do share.** Sharing is arranged rather than
  hoped for — generation 1 is a fresh `SobolSeq`, so the smaller run proposes a prefix of the larger
  run's points, and `cdf_grid_k=4` is above `k_min` for both population sizes.

---

## Session: an in-flight generation folder is not a completed generation (2026-09-06)

From the adversarial review of the calibration PRs (#33-#40); the last of its four high-severity
findings with a mechanical fix. `_buildEvaluateBatch` writes a generation's monad record before
launching any simulation, which `mkpath`s `generations/<t>/`, while the artifacts that define the
generation are written only by `_saveGeneration` at the end. So for the whole duration of generation t
— and permanently, if the run is interrupted — `_generationIndices` reported one generation too many:
`posterior(cal)` resolved `:final` to it and died "has no particle file", all four disk-based plot
styles failed the same way, and `show` over-counted. A regression from #39, the flat layout having
enumerated particle files, which made an in-flight generation invisible.

### Decisions
- **Two enumerations, named for the question each answers.** `_generationIndices` keeps its permissive
  scan, which migration and the monad-ID reader depend on; `_completeGenerationIndices` is what
  anything presenting a run to a user calls. Rejected: making the single function strict, which breaks
  `_migrateGenerationLayout!` (it must see the folder it is migrating) and `calibrationMonadIDs` (an
  interrupted generation's monads are real and were simulated).
- **`metadata.toml` is the commit marker, not `particles.csv`** — `_saveGeneration` writes the CSVs
  first and the TOML last, so the TOML is the only artifact whose presence implies the rest.
- **`_findLastGenerationCSVs` walks backwards** to the newest generation holding both artifacts rather
  than testing only the last index: same root cause, opposite symptom — it silently skipped the
  particle-consistency and LVSource-map validations on exactly the interrupted runs a resume targets.

### Not fixed here
`posterior` still says "has no particle file" for a *named* incomplete generation rather than
"generation N is incomplete". A run that stopped on `max_evaluations` crashes on resume for an
unrelated reason (issue #62).

---

## Session: two discrete-parameter defects from the calibration review (2026-09-06)

An adversarial review of the calibration PRs (#33-#40), the part of the 0.9 range that had had none.
Twenty-five findings survived verification; these are the two highest with a mechanical fix. The rest
became issues #61 (snapping bias) and #62 (triage).

- **Resume was dead for any discrete calibration, both routes.** #38 gave discrete parameters their own
  source types and let the default passthrough store them in `_ProblemManifest`, but neither resume
  path learned to read one back: `_sourceToCalibrationParameter` had methods for four source types and
  not these, so the automatic route reported the manifest *complete* and then died in
  `_manifestToProblem`, while `_validateStructuralMatch` fell through to "Unexpected saved source type"
  — the sharper failure, since that branch keys off the **saved** source type, so nothing a user
  re-supplies steers around it. Decision: reconstruct through the same constructor
  `_toCalibrationParameter` uses, so the rebuilt `LatentVariation` is the same `DiscreteUniform` over
  value indices; anything else risks a coordinate meaning a different level after resume. The
  structural check compares `values` element-wise and drops the `flip` comparison the DV/CV branches
  make, and since a saved coordinate *indexes* the level list, a reordered or resized list silently
  re-points every particle. **Trap:** the test that looked like coverage asserted only
  `loaded.sources[1] isa DiscreteSource` while its comment claimed "so a discrete run can resume".
- **A discrete co-variation's inverse map checked only the first target**, where the continuous
  co-variation twenty lines below recovers `u` from the first and verifies the rest agree. So the
  `SimulationBank` would admit a row whose remaining co-varied columns are paired with the wrong index,
  and with `cdf_grid_k` set `_lookupAndSnap` can serve that monad to a particle whose reported
  parameters say otherwise. Decision: mirror the continuous guard exactly, `NaN` for "off the curve"
  rather than throwing. **Rejected: a tolerance comparison** — the continuous branch needs one because
  it round-trips through `quantile`, while a discrete level is copied verbatim, so a tolerance would
  admit a neighbouring level. **Not fixed here:** Phase 2 of the bank tests a discrete column against
  `minimum`/`maximum` of its levels rather than `insupport`, so an off-level value inside the range
  still clears that stage (issue #62).

---

## Session: a discrete coordinate is not snapped to the CDF grid (2026-09-06) — ships in v0.10.0

Issue #61. `_snapToCDFGrid` snapped every proposal coordinate to `{j/2^k_eff}` with no knowledge of
what the coordinate meant. A discrete parameter is a `DiscreteUniform(1, L)` over value indices and the
dyadic grid is spread evenly over [0, 1] rather than over the `L` level bins, so levels received
unequal numbers of grid points — and when `2^k_eff < L`, some received none. Measured for `L = 10` at
`k_eff = 3`, which `k_min` produces for `population_size=100` in three dimensions and so is the
*default* resolution: generation 1 reaches levels 2-5 and 7-9 and never proposes 1, 6 or 10. Generation
1 weights every accepted particle equally and sets the next generation's epsilon from that population,
so nothing downstream corrects it.

### Decisions
- **Do not snap a coordinate whose latent prior is discrete.** `_snapToCDFGrid` gains a three-argument
  method dispatching on `DiscreteDistribution`, and `_lookupAndSnap` takes `priors` — already threaded
  into both generation runners and previously ignored. Snapping buys a discrete coordinate nothing: the
  quantile already collapses each level's whole CDF bin to one target value, and what prevents a repeat
  simulation is `use_previous=true` in `_createMonadForParams`, where two proposals in one level share
  a `variation_id` and resolve to the same monad. Dispatching on the prior rather than a precomputed
  mask puts the rule next to the arithmetic instead of adding another array to keep aligned with
  `param_names`.
- **Bank lookup is untouched, and needed to be** — `_lookupAndSnap` always tested the box against
  `raw_cdf`, the *unsnapped* proposal, so the discrete axis's bank behaviour is bit-identical. The one
  second-order effect is on `mid_gen_additions`, which now stores a discrete coordinate's raw value:
  one extra `evaluate_batch` entry and no extra simulation.

### Rejected
- **Snapping a discrete coordinate to its level midpoint `(2i-1)/(2L)`** — the canonical value it keeps
  matters only for `_cdfToGridKey`, which no production path calls.
- **Warning when `2^k_base_eff < L`** — a warning about a distortion we can simply not create.
- **Raising `k_base_eff`** — it shrinks the relative imbalance but never removes it (`L = 3` divides no
  power of two) and inflates the proposal count for no extra resolution.

### Traps
- **`DiscreteDistribution` is not a `Distributions` name `abc_smc.jl` had in scope** — the file imports
  selectively and it worked only because `variations.jl` does a blanket `using Distributions` into the
  same module. Named explicitly now.
- **The regression test needs the database** even though it is algorithm-level, `_runFirstGeneration`
  calling `_updateMidGenAdditions!` after `evaluate_batch`, which queries simulation statuses.
  Generation 1 is the clean place to assert it: an empty bank and an empty `mid_gen_additions` mean no
  lookup can fire, so the proposals are exactly the snapped Sobol points.

---

## Session: the nineteen remaining calibration findings (2026-09-06) — ships in v0.10.0

Issue #62: what was left of the adversarial review of the calibration PRs after the four highest
findings were split off. Nineteen items in four groups, each re-verified against the code before it was
touched; three that were design questions rather than defects were left for their own briefs.

### Decisions
- **A resume recomputes `budget_hit` rather than trusting the last generation.** Every other stopping
  criterion is a property of the generation just finished; the budget spans the run and nothing on disk
  records that it was spent, so `_stoppingReason`'s budget branch was unreachable from a resume and the
  crash it produced (`maximum` over an empty vector) never named the budget. Its warning is
  unconditional, unlike the `@info` for the other reasons: a resume that can run nothing is a dead end.
- **A generation that accepts nothing is discarded, never persisted** — the next generation resamples
  from this one's particles, so a zero-particle generation is broken rather than small. The warning
  reports the closest distance seen against ε, the one number that discriminates "budget spent" from
  "ε unreachable", which otherwise always arrive together. `method.toml` is likewise written only once
  the resume commits to running a generation.
- **`_supportSize` uses `length(support(d))`.** The span `maximum - minimum + 1` is right only for a
  contiguous unit-step support — which is what the discrete path builds for itself, hence the bug's
  long life — and the grid walk indexes `collect(support(d))` with the result, so a gappy support ran
  off the end. Relatedly a **single-level discrete parameter is rejected**: it cannot vary, its latent
  `DiscreteUniform(1, 1)` still costs a kernel dimension that dilutes the fitted covariance, and
  nothing in the output would say so, the posterior column being simply constant.
- **The bank's interior filter became per latent dimension** — `0 < u < 1` is a statement about a
  continuous prior, while a discrete top level has `cdf` exactly 1. Checked that admitting 1.0 is safe
  downstream (L∞ box comparison, `quantile(DiscreteUniform(1, k), 1.0) == k`, kernels confined to
  [0,1], prior density 1 everywhere in CDF space) before widening it.
- **The negative-axis fix moved the axis, not the bins** — the recipe passes one `bar_width` for every
  bar, so uniform bin width is load-bearing and an existing test asserts it; clamping the first edge was
  written, caught by that test, and reverted for an `xlims` default that starts at the data. A
  non-finite proposal distance is dropped and counted in the title rather than clamped, clamping being
  a distance the run never measured in a plot about where they fell.
- **`tags=` accepts a lone `Pair` in `runCalibration` only** — `_asTagCollection` would fix `run` and
  `createTrial` in a word each, but a triage PR is not the place to change `run`'s keyword handling.
  Calibration tag keys are validated before `createCalibration`, so a typo leaves no orphan row.

### Rejected
- Erroring rather than stopping gracefully when a generation accepts nothing — an exception would also
  discard the `ABCResult` for the generations that did succeed.
- Distinguishing "budget already spent" from "spent mid-generation" in the empty-`accepted` message —
  with the resume-time check fixed, the first is unreachable.
- Keeping `_DistanceData.max_epsilon_accepted` "for future use" — written by two call sites, read by
  none.

### Traps
- **A `Pair` is iterable** — `tags = "a" => "b"` splatted into `tag!` becomes two valueless tags,
  silently, both being legal keys. So is a `String`, one tag per character.
- **A recipe that declares keyword arguments cannot be applied without a plotting backend**:
  RecipesBase's cleanup calls `is_key_supported`, which only Plots defines, which is why the
  `ABCResult` and `Calibration` recipes had no direct coverage. The suite now defines it as `true`.
- **`normalizeTagPairs` returns tuples, not pairs**; feeding its output back into `tag!` hits
  `_tagPair`, which has no `Tuple` method.
- **The generations a result holds are not indexed by generation number** — `_loadGenerations` skips a
  generation whose CDF file is missing, so `generations[i].t == i` holds only until a resume follows an
  interrupted write, which is exactly when a plot is being read.

---

## Session: assertions whose two arms coincided (2026-09-06) — ships in v0.10.0

Issue #59: four places where the suite could not fail if the behaviour regressed, plus the fact that no
`run()` had ever reached the SLURM branch. Tests only.

### Decisions
- **`TestSimulator.runSimulation` now creates the simulation's `output/` folder.** That absence was the
  root cause of the dead `verifyStoredValues` branch: `!isdir(pathToOutputFolder(sid))` classed every
  simulation unverifiable, so `n_agreed` was 0, `n_agreed + n_unverifiable == length(sids)` held
  trivially, and the mismatch block was skipped by its own guard. Fixed in the stub, not the testset,
  because a real backend leaves an output folder behind — the stub was unfaithful.
- **Reducer tests measure the simulation ID**, every replicate of a monad sharing its parameter values,
  so `mean` and `maximum` of a measurement of those values are the same number. Likewise `run_kwargs`
  precedence is observed by a callback that counts itself, both arms having been `on_progress=nothing`.
- **The `StudySpec` override is counted in new simulation rows, not in a monad's constituents.** MOAT is
  deterministic for a given set of variations, so a second sweep lands on monads an earlier sweep built,
  whose constituent lists carry that run's replicates too; `use_previous=false` guarantees
  `n_replicates` *new* simulations per monad, not a total.
- **A `run()` success case on the SLURM path sits beside the refusal test** and picks up the simulations
  that refusal left pending, which is what makes the refusal test's claim worth something — the first
  coverage of what `run`'s worker does with the `SimulationProcess(process=nothing, success=true, cmd)`
  a submitted job returns. Timing-shaped assertions wait on `hpc.out` carrying the job id, the event
  the fixed sleeps stood in for, and `_await` bounds every `fetch`.

### Rejected
- **Capturing stdout to assert `quiet=true` also wins** — `redirect_stdout` takes no `IOBuffer` on 1.12,
  so this needs a `Pipe` plus a reader task around a whole calibration, for a second assertion about
  the same one-line precedence rule.
- **Asserting a per-monad constituent count for the `StudySpec` override** — not an invariant, monads
  being shared between sweeps.

### Traps
- **`GenerationResult.particles` holds CDF coordinates, not target values**;
  `posterior(result; generation=t)` is the display-value view and carries `distance` and `monad_id`
  alongside, so it is the single source for both halves of such an assertion.

---

## Session: recovering a campaign whose driver died (2026-09-06) — ships in v0.10.0

Issue #53, the remainder of the 0.9 HPC architecture review. The Julia process that calls `run` is the
only thing that records outcomes; when it dies, the jobs finish, nothing writes their results, and the
rows stay `Running` forever — and since `isStarted` counts everything but `Not Started` as started,
every later run skips them *and* reports that it is saving the user time. The only recovery on offer
was `deleteSimulationsByStatus`, which deletes rows and output folders and recycles the IDs.

### Decisions
- **Reconcile inside `databaseDiagnostics`, not as a standalone function.** Diagnostics already runs at
  every `initializeModelManager` and already reports orphaned rows and staged `.trash` paths, so a
  stranded `Running` row is the same kind of finding, while a separate entry point is one the affected
  user has to know to call. It writes, which the file's own comment says a function named "diagnostics"
  should not — accepted deliberately and said in its docstring, the alternative being a report about
  rows nothing will ever fix.
- **Two sources, in that order: the sentinel, then `sacct`.** The sentinel is the file the worker would
  have read, so it gives the exit code directly; `sacct` reads slurmdbd rather than the queue, which is
  what lets it answer for a job that left hours ago, where `squeue` — the runner's reaper — cannot.
  `COMPLETED` is success, the failure and waiting lists cover every state SLURM currently documents,
  and anything else is reported by name and left at `Running`, the lists not being able to be
  exhaustive. One `sacct -j <all ids> -n -P --format=JobID,State` per pass rather than one per
  simulation, slurmdbd being a second daemon with a database behind it; `.batch`/`.extern` step rows
  are dropped, a step's state being able to differ from its allocation's.
- **Sentinels are ordered by the stamp in the name, and the stamp is wall clock.** The row still at
  `Running` belongs to the *later submission*, which `mtime` gets backwards whenever the
  earlier-submitted job outlived the later one — quite apart from `mtime` being unreliable on a network
  filesystem. The stamp moved from `time_ns()` to `round(UInt64, time() * 1e9)` because `time_ns()`'s
  epoch is per-boot, so stamps from two driver sessions were never comparable. A sentinel is **read,
  not consumed**: diagnostics cannot tell an abandoned simulation from one another live session is
  still waiting on, and consuming that session's sentinel would leave its worker to be failed by the
  reaper minutes later.
- **The sentinel directory is fixed per session and moved only by an environment variable.**
  `HPCCompletionOptions.done_dir` is gone — a setter could move it mid-session, and diagnostics reads
  it at initialization. `MODELMANAGER_HPC_DONE_DIR` is read once by `initializeModelManager` onto
  `ModelManagerGlobals.hpc_done_dir`; the default `data/outputs/.hpc_done` sits on the filesystem the
  jobs already write into, so it is writable by construction. A given value is `mkpath`ed and
  write-probed at init, throwing an `ArgumentError` naming the variable, because a directory the
  compute nodes cannot write makes *every* successful job look scheduler-killed, silently, and that is
  the one check a login node can make.
- **`useHPC` pins the flag for the session.** `run_on_hpc_overridden` is consulted by
  `initializeModelManager` before it re-seeds `run_on_hpc` from the probe; without it a downstream
  package's `__init__`, or any re-initialization, would put the probed value back and a `useHPC(false)`
  at the top of a script would be gone before its first `run`. Session state, not project state.
- **A template, not a `submitDriver` function.** Submission is one line of `sbatch`; what has to go
  *above* it is site knowledge ModelManager does not have, and loading Julia plus a downstream package
  to do that string substitution is minutes on a cluster. `initializeModelManager` and `useHPC(true)`
  write `driver_template.sbatch` into `<project root>/scripts/` if that folder exists and the project
  root otherwise — `dirname(dataDir())` either way, so it sits beside the scripts a user edits rather
  than inside the `data/` the deletion helpers sweep. Written once, never overwritten. Its `--time`
  comment names the trap: the limit must cover the whole campaign *including* the queue waits of every
  simulation job the driver submits.
- **Nested submission stays.** Inside the driver job, detection stays on and each simulation is still
  its own job, which is the design; the review's proposed hint at `useHPC(false)` when `SLURM_JOB_ID`
  is set would have told users to undo it, so the template says so in a comment instead.
  `_SubmissionRefused` accordingly stays internal, and `hpc_completion.jl` leaves the HPC reference
  page's `Pages` list, nothing in the file being public.
- **`Queued` and `Running` mean what `run` sets them to.** `run` marks every simulation it is about to
  run `Queued`, and the worker that claims one marks it `Running` before calling the backend and so
  before any `sbatch`. The old reconciler had this backwards: a `Queued` row was checked for an
  `hpc.out` it can only have from an earlier run and would have stayed `Queued` forever, and a
  `Running` row with no sentinel and no job ID was left alone when on HPC that is proof no job exists.
  Both now return to `Not Started`; off HPC the latter is left, what a local process did after its
  session ended not being knowable.
- **The stranded ID sets are taken synchronously at init.** Diagnostics runs in an `@async` task after
  `initializeModelManager` returns, so a `run` on the next line can have marked this session's own
  simulations `Queued`/`Running` — exactly the two statuses treated as evidence of a dead driver. Two
  cheap queries at init instead; a hand call still queries live, which is safe mid-run and documented
  as such.

### Rejected
- **A job array keyed by simulation ID** — every task shares one resource request, right for a sampling
  and wrong for a mixed trial, and the reaper would have to understand `jobid_taskid`. Worth its own
  brief if per-job submission overhead ever becomes the bottleneck.
- **A probe job to validate the sentinel directory on first submission** — only a compute node can
  prove the directory is writable from a compute node, but that means submitting and waiting on a job
  before every campaign.
- **A `setHPCDoneDir` setter, or a `done_dir` back on `HPCCompletionOptions`** — either lets the
  directory move after diagnostics has read the old one, the failure the fixed location prevents.
- **Resolving the monad before recording a reconciled failure** — `updateDatabaseOnCompletion` and
  `simulationFailed` take `Union{Missing,Int}`, so `missing` reaches the lookup
  `eraseSimulationIDFromConstituents` already does; duplicating it would be a second place to keep in
  step.

### Traps
- **`useHPC` in the test suite is now sticky.** Every testset that flips it leaves the pin set, so a
  later `initializeModelManager` keeps that value instead of re-probing; the SLURM testsets clear
  `run_on_hpc_overridden` before initializing, and anything new depending on the probe must too.
- **`databaseDiagnostics` is `@compat public`, not exported** — a test calling it bare gets
  `UndefVarError` in `Main`.
- **The `sacct` shim needs `sacct.out` cleared between subtests**, or a leftover state string decides a
  later subtest's outcome; `_reset_hpc!` removes it.
- **Test projects now nest their data directory.** `initializeModelManager` takes the *data* directory
  and the template is written to its parent, so a test passing `mktempdir()` itself would drop
  `driver_template.sbatch` into the system temp directory. The HPC blocks use `<mktempdir()>/data`.
...
- **Reserved sbatch keys are refused in `setJobOptions`**, not asserted at the first submission; so are
  non-`String` keys (review).
- **`cpus-per-task` is a default, filled by the backend.** Only the backend knows its thread count, so
  ModelManager stubs `simulationThreads(sim, simulation)` (default `nothing`, which omits the flag) and
  `defaultJobOptions` requests it per submission. Every `Function`-valued job option now receives the
  `Simulation`, matching the measurement contract.
- **Ownership is tracked, not inferred.** Review found the shutdown reset using `istaskstarted` as "no
  worker took this", which lies between a worker's dequeue and its `schedule`, so a retried `run` could
  schedule the same simulation twice. Workers now mark a `claimed` bit the instant they dequeue an
  index, with no yield in between.
...
Decision: the predicate asks the question that matters -- can a fresh session restore this by name? --
and it is JLD2's own answer, `occursin('#', string(typeof(f)))`, the test JLD2 applies before warning
that it only stores functions by name. Review asked whether JLD2 had already solved this rather than
us guessing; it had, and the one-liner agrees with the hand-rolled three-step version on every case in
the test (top-level, `Base`, lambda, inner named, `let`-scoped, capture-free inner, callable struct).
Spelled locally rather than calling `JLD2.isgensym`, which is internal. `_loadProblem` takes
`required=`, so with a `problem=` in hand an unreadable file -- absent, without a `manifest` entry, or
unreconstructable -- is a warning and the supplied problem is used unvalidated; without one it is an
error naming both ways out. Rejected: validating the supplied problem against `parameters.toml`
instead of the manifest -- more machinery than the case warrants today.
...
- **A `Vector` is not spread by index.** The only cross-monad check would be length, and equal length is
  not equal meaning. *Revised in the #70 review (2026-09-06):* the length argument is naive — a vector
  you wrote is perfectly reasonable to reason about — and is gone from the docs and the message. The
  refusal stands as a "for now"; see the 2026-09-06 QoI-contract entry.
