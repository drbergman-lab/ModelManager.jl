...
#! are checked: a named `compute` with an anonymous `reduce` would otherwise round-trip as a QoI that
#! silently averages instead of doing the monad-level step it was written for.
