...
#
# This file wires the generic ABC-SMC loop to the ModelManager trial infrastructure.
# A proposed parameter vector becomes a Monad via addVariations, which is then run
# with ModelManager.run. The summary_statistic and distance functions are supplied
# by the caller (in CalibrationProblem) and may be simulator-specific.
#
...
creates a Monad, runs it quietly (per-sim/per-trial console output suppressed via
`run(monad; quiet=true)` so generation-level progress dominates the console), records
the monad ID, and computes the distance via `problem.summary_statistic` and
`problem.distance`.
"""
...
The calibration is tracked in the database (`calibrations` table) and all monads
...
Save the ABCSMC settings to `method.toml` in the calibration output folder for resume support.
"""
