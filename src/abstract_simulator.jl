...
a SLURM job; override it only for a simulator that is not an external process. On SLURM the default
asks the backend how many CPUs to request through [`simulationThreads`](@ref), which defaults to
"no request"; implement it for a simulator that decides its own thread count.
...
`prepareTrialHierarchy` after [`setupSampling`](@ref) has already run for
...
by `prepareTrialHierarchy`. Return `true` on success, `false` on failure.
...
`version`. Called by `upgradePackage` for each milestone that needs to be
...
Called by `processSimulationTask` for every completed simulation. This is the slot
...
Called by `processSimulationTask` for every completed simulation, regardless of
...
"""
    simulationThreads(sim::AbstractSimulator, simulation::Simulation) -> Union{Nothing,Int}

How many threads `simulation` will start, so the runner can ask SLURM for that many CPUs
(`--cpus-per-task`). The default, `nothing`, requests no CPU count and leaves the site's default in
force -- usually one.

Implement it for a simulator that decides its own thread count. PhysiCell, for example, reads
`omp_num_threads` from its configuration and starts that many threads whatever SLURM allocated, so
without this every job time-slices its threads on one core. Called once per submission with the
[`Simulation`](@ref) about to be submitted, so a varied thread count is honoured. A user's own
`setJobOptions(Dict("cpus-per-task" => …))` replaces the default and is never overridden.
"""
simulationThreads(::AbstractSimulator, simulation) = nothing

########################################################
############        Public API surface       ###########
########################################################

#! These names are deliberately *not* exported — a simulator package implements
#! `ModelManager.runSimulation`, it never calls an exported one — but they are public API:
#! they are the documented contract in "Building a Simulator Backend", and downstream
#! docs builds must be able to resolve `@ref` links to them. Declaring them public makes
...
@compat public runSimulation, simulationCommand, simulatorDir, simulatorVersionSchema, simulatorVersionIDName,
               ...
               getInputFolderDescription, clearSimulatorArtifacts, simulationThreads
