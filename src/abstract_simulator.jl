...
"""
    addVariationRows(sim::AbstractSimulator, inputs::InputFolders, reference_variation_id::VariationID, loc_dicts::Dict)

Write new variation rows to the simulator's per-location variation databases for `inputs` and
return a `Vector{VariationID}` — one entry per column of values supplied in `loc_dicts`.

`loc_dicts` maps each varied location `Symbol` to a tuple
`(values_matrix, types_vector, targets_vector)` where `values_matrix` is
`n_targets × n_samples`.

Called by [`addVariations`](@ref) (Grid, LHS, Sobol, RBD) after the generic sampling logic
computes the CDF-derived parameter values.  Implementations are responsible for:
1. Ensuring the required columns exist in the per-folder variation DB.
2. Inserting rows (or returning the existing ID when a row already exists).
3. Returning one `VariationID` per sample point.
"""
function addVariationRows end
