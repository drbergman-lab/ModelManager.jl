    ...
    @testset "DB-backed integration" begin
        mktempdir() do project_root
            ...
            @testset "runCalibration end-to-end" begin
                ...
                # An in-flight generation folder (only its monad record written) is never `:final`,
                # and asking for it by number says what it is.
                done     = ModelManager._completeGenerationIndices(gen_dir)
                inflight = joinpath(gen_dir, lpad(last(done) + 1, ndigits(3), '0'))
                mkpath(inflight)
                write(joinpath(inflight, "monads.csv"), "1\n")
                try
                    live = samplePosterior(result.calibration, 5)
                    @test issubset(live.monad_id, result.generations[end].monad_ids)
                    @test_throws ArgumentError samplePosterior(result.calibration, 5;
                                                               generation=last(done) + 1)
                finally
                    rm(inflight; recursive=true, force=true)
                end

                # Draws become a runnable Sampling. A plain draw resolves to the monad it came
                # from, so nothing new is created ...
                cal_monads = ModelManager.calibrationMonadIDs(result.calibration)
                plain      = samplePosterior(result, 6; rng=MersenneTwister(9))
                s_plain    = createTrial(result, plain)
                @test s_plain isa Sampling
                @test sort(monadIDs(s_plain)) == sort(unique(plain.monad_id))
                @test issubset(monadIDs(s_plain), cal_monads)

                # ... while a smoothed draw is a new parameter set and gets a new monad, whose
                # recorded parameter value is the drawn one.
                sm   = samplePosterior(result, 3; smooth=true, rng=MersenneTwister(10))
                s_sm = createTrial(result, sm)
                @test length(monadIDs(s_sm)) == 3
                @test isempty(intersect(monadIDs(s_sm), cal_monads))
                col = string(columnName(xp_x))
                @test sort(simulationsTable(s_sm)[!, col]) ≈ sort(sm[!, col])
                run(s_sm; quiet=true)
                @test all(==(ModelManager.statusCodeID("Completed")),
                          values(ModelManager._simulationStatusIDs(simulationIDs(s_sm))))

                # From disk, the same draws resolve to the same monads; a frame without the
                # parameter columns is refused, as is an empty one.
                @test sort(monadIDs(createTrial(result.calibration, sm))) == sort(monadIDs(s_sm))
                @test_throws ArgumentError createTrial(result, DataFrame(monad_id=[1, 2]))
                @test_throws ArgumentError createTrial(result, sm[1:0, :])
            end
