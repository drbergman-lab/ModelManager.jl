...
- `samplePosterior(result_or_calibration, n; generation=:final, smooth=false, rng)` draws `n` parameter sets from a generation's posterior and returns a `DataFrame` of the same display columns `posterior(calibration)` gives — the parameter columns alone, without the `weight` and `distance` bookkeeping that `posterior(result)` happens to carry, since a draw's weight is `1/n` by construction and leaving the particle's weight in the frame would invite a second weighted average over an already-weighted sample. **Plain mode (`smooth=false`, default)** resamples the accepted particles i.i.d. with probability equal to their importance weights; every draw is an existing particle, so the frame carries a `monad_id` column and a posterior predictive check can reuse the monad's outputs without a new simulation. **Smoothed mode (`smooth=true`)** turns the weighted particles into a Gaussian kernel density estimate in CDF space and draws from it: resample a parent, add `N(0, h² Σ_w)` noise where `Σ_w` is the weighted particle covariance and `h = ESS^(-1/(d+4))` is Scott's factor with the effective sample size in place of `N` (the `perturbation_kernel`'s scale is a proposal width, tuned for exploration, and is too wide for a density estimate), reflect at the `[0, 1]` boundary rather than reject (rejection thins the edges and there are no importance weights here to correct it), then map through each prior's quantile exactly as `_particleRowToDisplay` does. Working in CDF space is what keeps draws inside prior support, respects log-scale priors, and makes discrete parameters snap to a level. Smoothed draws are new parameter sets, so no `monad_id`. A diagonal floor of `1e-10` keeps the covariance factorisable when a coordinate has collapsed. On a `Calibration`, plain mode reads `{t}/particles.csv` and needs nothing else; smoothed mode reads `{t}/cdfs.csv` and rebuilds the parameters from `problem.jld2`, so it errors when a `LatentVariation` was saved with anonymous maps, naming `resumeABC(cal; problem=)` as the route to an `ABCResult`.
...
- `samplePosterior(result, n)` returns `n` rows, each equal to one accepted particle, with draw frequencies matching the weights; a zero-weight particle is never drawn. `samplePosterior(result, n; smooth=true)` returns `n` rows inside every prior's support (discrete parameters on a level) with no `monad_id` column, and collapses onto the particle when the generation has one particle.
