...
- `simulationCommand(sim, spec::SimulationSpec)` → `Cmd` — the command that runs one simulation
...
- `runSimulation` has a default: it asks the backend for the command via `simulationCommand(sim, spec)::Cmd` and owns everything else — output folder, `output.log`/`output.err`, working directory, and local-vs-SLURM dispatch. A backend overrides it only if its simulation is not an external process. On HPC each simulation is wrapped in an `sbatch --wrap` invocation submitted with `--parsable`; see the HPC Job Completion feature for how the runner learns it finished.
...
- The generated `--wrap` text, run under `/bin/sh`, records the true exit code at the given path and preserves the script's own status.
...
- **`QoI(name, compute; reduce=mean)` is the seam between a user's measurement and the three things that consume one.** `compute` is called with one `Simulation`; `reduce` combines a parameter set's replicate values. The `QoI` is passed *directly* to `run(::GSAMethod, ...; functions=)`, to `CalibrationProblem`'s `summary_statistic`, and to `run(...; post_processor=)` — there are no adapter functions in the public API.
  ...
  - **Sensitivity analysis honours the reducer.** `evaluateFunctionOnSampling` previously hard-coded `mean`; it now takes the reducer from the QoI, which is the same question calibration asks — summarise this group of simulations. The `Float64` result type stays, because the sensitivity indices require it, so a reducer returning something else fails at the QoI rather than inside `GlobalSensitivity`.
  - **A plain `Function` keeps its exact contract**: called with a simulation *ID*, replicates averaged with `mean`. Only a `QoI`'s `compute` receives a `Simulation` object. Wrapping existing functions into `QoI`s would silently change what every `functions=[f]` call is handed.
  - **The value constraint comes from the consumer, and it does not fall on the same function in each case.** Sensitivity analysis needs `reduce` to return a `Real`; calibration needs `reduce` to return whatever its `distance` accepts; the **sink constrains `compute` instead**, because it fires once per simulation, so `reduce` is never called and `compute`'s own value is stored. A `compute` returning a vector or `Dict` is therefore fine for the first two and rejected by the third — with an error naming the QoI and the offending type.
