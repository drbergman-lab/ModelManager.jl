...
- `packageName(sim)` and `loadedPackageVersion(sim)` both default to the package defining `typeof(sim)`, resolved through `Base.moduleroot` so a type in a submodule reports its package. Both are overridable, for the case where the simulator type lives in a different package from the one the database tracks.
- `packageName` is the single place deciding which package is version-checked: `getInstalledVersion` looks that name up in the environment and `loadedPackageVersion` looks it up in `Base.loaded_modules`, so the two always describe the same package and overriding `packageName` moves both. The installed version is never substituted for the loaded one. `loadedPackageVersion` returns `nothing` when the named package is not loaded or carries no version.
