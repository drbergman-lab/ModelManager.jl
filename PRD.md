...
- When the simulator type belongs to no package, `loadedPackageVersion` falls back to the *loaded* version of the package `packageName` names, found in `Base.loaded_modules`. Both routes report a loaded version; the installed version is never substituted for it. `nothing` means neither route found one.
