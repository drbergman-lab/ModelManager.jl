...
- **`QoI` is the seam between a user's measurement and the three things that consume one.** Sensitivity analysis wants a function of a simulation ID, calibration wants a function of a monad ID, and the post-processing sink wants a function of a `SimulationProcess`. `QoI(name, level, compute; reduce=mean)` is the measurement written once; `sensitivityFunction`, `summaryStatistic` and `postProcessor` adapt it. Purely additive — no consumer changed, because each adapter returns exactly the callable that consumer already accepted.
  - **The level is declared as a type, not inferred and not a `Symbol`.** `Simulation` and `Monad` are both `AbstractMonad`s, so a `compute` written for `AbstractMonad` is callable at either level and dispatch cannot recover which was meant; reading a monad as a simulation is the failure this seam must make impossible. A type also makes the declaration typo-proof (`Simulaton` is an `UndefVarError`, `:simulaton` would be a silent mismatch) and lets the adapters dispatch on `QoI{Simulation}` versus `QoI{Monad}`, so illegal pairings are unrepresentable rather than validated. `hasmethod(compute, Tuple{L})` at construction therefore *verifies a declaration* rather than guessing one.
  - **`reduce` collapses replicates, and matters only at `Simulation` level.** Calibration needs one number per parameter set, so a simulation-level QoI reduces across the monad's simulations. A monad-level `compute` sees every replicate itself.
  - **Two combinations are refused rather than silently wrong.** Sensitivity analysis averages replicates with a hard-coded `mean`, so a simulation-level QoI carrying a different `reduce` would have it discarded without trace — `sensitivityFunction` throws instead. And a monad-level QoI has no per-simulation form for sensitivity analysis to call. `postProcessor` excludes the monad level in its *signature*, since the hook fires once per simulation and no monad exists to hand it.
