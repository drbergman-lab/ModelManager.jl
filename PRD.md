...
- **A `QoI` can read a value the post-processing sink stored earlier**, via `stored=:prefer` (stored if present, else compute) or `stored=:require` (stored or error). This is the write-once-read-later path that matters once post-simulation cleanup has deleted whatever the quantity was computed from.
  - **A stored value is read back the way it was written.** A scalar comes back as the sink holds it (`Float64`, `Int64` or `String`, never coerced to `Float64`); a keyed QoI, spread by the sink into `"<name>.<key>"` columns, is reassembled into a `Dict` with `String` keys. `verifyStoredValues` compares numbers with `isapprox`, keyed values key by key, other values with `isequal`, and counts a `compute` that returns `missing`/`nothing` as unverifiable.
