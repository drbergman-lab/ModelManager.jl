...
- Return-value contract:
  ...
  - **Each key becomes the column `"<qoi name>.<key>"`**, not a bare column — the same rule sensitivity analysis uses for a spread `reduce`, so one measurement names its parts identically wherever it is consumed. Two QoIs that both report a `tumor` therefore no longer land in one column. A scalar return keeps using the QoI's name alone.
  - **A bare anonymous function cannot write to the sink at all.** Its name is a regularised gensym (`anon_9`) that varies between sessions, and since it now prefixes every column the same script would write a fresh, half-empty set of columns each run. Wrapping it (`QoI("counts", f)`) or passing a named function is the fix; returning `nothing` is unaffected, since nothing needs naming. This closes the previous exemption, under which a `NamedTuple`/`Dict` return from an anonymous function was allowed because the QoI's name was never consulted.
  - any other return type, or a non-scalar value → `ArgumentError`.
  - quantity names are quoted safely for SQL (interior `"` doubled); an `AbstractDict` whose keys collide after string conversion (e.g. `1` and `"1"`) → `ArgumentError`, still, because the prefixed pairs reach the sink as separate ordered entries rather than being collapsed into a `Dict` first.
...
- **`QoI(name, compute; reduce=mean)` is the seam between a user's measurement and the three things that consume one.** `compute` is called with one `Simulation`; `reduce` combines a parameter set's replicate values. The `QoI` is passed *directly* to `run(::GSAMethod, ...; functions=)`, to `CalibrationProblem`'s `summary_statistic`, and to `run(...; post_processor=)` — there are no adapter functions in the public API.
  - **There is no level to declare, because per-simulation `compute` plus `reduce` is strictly more expressive than a monad-level `compute`.** A monad-level measurement is that pair with the reduction folded into `compute`, so it added a granularity question — and the `Simulation`-versus-`Monad` dispatch collision with it — for no expressive gain. Measurements needing the replicates *jointly* rather than as summarised numbers are expressed by having `compute` return the raw material and `reduce` do the pooled work, since `reduce` receives everything `compute` returned.
  - **Sensitivity analysis honours the reducer.** `evaluateFunctionOnSampling` previously hard-coded `mean`; it now takes the reducer from the QoI, which is the same question calibration asks — summarise this group of simulations. Each *component* of the reduced value must be a `Real`, since it becomes a sensitivity index, so anything else fails at the QoI naming it rather than as a bare `convert` error inside `GlobalSensitivity`.
  - **A plain `Function` is wrapped into a `QoI` at the boundary**, gaining the two things it lacks: a name (from `_qoiNameFromFunction`, so an anonymous one becomes `anon_…`) and `reduce = mean`. Nothing downstream branches on which it was given.
  - **A keyed value is spread by both the sink and sensitivity analysis, under one naming rule: `"<qoi name>.<key>"`.** The dot rather than an underscore because `_` is legal inside both a QoI name and a key, making `a_b_c` ambiguous between QoI `a` + key `b_c` and QoI `a_b` + key `c`.
  - **The value constraint comes from the consumer, and it does not fall on the same function in each case.** Sensitivity analysis needs `reduce` to return a `Real` or a `Dict`/`NamedTuple` of them; calibration needs `reduce` to return whatever its `distance` accepts; the **sink constrains `compute` instead**, because it fires once per simulation, so `reduce` is never called and `compute`'s own value is stored. A `compute` returning a vector or `Dict` is therefore fine for the first two and rejected by the third — with an error naming the QoI and the offending type.
- **A `QoI` can read a value the post-processing sink stored earlier**, via `stored=:prefer` (stored if present, else compute) or `stored=:require` (stored or error). This is the write-once-read-later path that matters once post-simulation cleanup has deleted whatever the quantity was computed from.
...
- `resumeCalibration(calibration::Calibration, method=nothing; kwargs...)` resumes from saved generation files, with `resumeABC` as its ABC-specific alias. No `problem` argument required — the `CalibrationProblem` is loaded from `problem.jld2`.
  ...
  - **The effective settings are written back to `method.toml`** whenever they differ from what is stored, with the changed keys reported. Without this the file would describe a run that no longer matches it, and the *next* resume would silently revert to the original values. The write happens only once the resume is committed to running a generation — after the problem is validated and after the stopping criteria are checked — so a resume that errors, or that has nothing left to do, leaves the file describing the run that did happen.
