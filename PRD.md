...
- `initializeModelManager` seeds `run_on_hpc` from `isRunningOnHPC()` (a probe for `sbatch` on the `PATH`) on every call, placed after all early-return failure paths and before `postInitDisplay` prints it. `useHPC(use)` overrides it and sets `run_on_hpc_overridden`, which suppresses the probe on every later `initializeModelManager` in the session, for every project. Without the pin, a simulator package that initializes a project in its own `__init__`, or a script that re-initializes, would silently undo the override.
...
- After `initializeModelManager` returns `true`, `mm_globals().run_on_hpc == isRunningOnHPC()` — unless `useHPC` has been called, in which case it keeps the value that call set, however many times the project is re-initialized.
...
- Files nothing can still be waiting on (crashed-driver sentinels, staged `.tmp` writes from killed jobs, `.nfsXXXX`) are swept once per `reap_interval`, age-gated to `max(4·grace_period, 1h)` so a sentinel directory shared across sessions is safe.
- `setHPCCompletionOptions(; submit_retry_period, poll_interval, reap_interval, grace_period)`. The sentinel directory is **not** among them: it is always `data/outputs/.hpc_done`. A configurable one bought the ability to escape NFS's 30–60 s directory-attribute caching, and cost the failure where a directory the compute nodes cannot write makes every successful job look scheduler-killed, silently. Latency is the lesser failure.
- **A driver that dies leaves recoverable state, and `databaseDiagnostics` recovers it.** For each simulation still at `Running`: the newest sentinel named `<simulation_id>.*` decides the outcome if one is on disk (read, not consumed, so a worker in another live session still resolves normally; the age-gated sweep reclaims it); otherwise the job ID in that simulation's `hpc.out` is put to `sacct -j <id> -n -P --format=State,ExitCode` when `sacct` is on the `PATH`. `COMPLETED` records success; `FAILED`/`TIMEOUT`/`CANCELLED`/`OUT_OF_MEMORY`/`NODE_FAIL` record failure through the same `updateDatabaseOnCompletion` the runner uses, so a failure is erased from its monad exactly as a live one would be; `PENDING`/`RUNNING`/`SUSPENDED` are left alone; any other state is reported and left alone. Each simulation at `Queued` with no `hpc.out` was never submitted and returns to `Not Started`. Counts are reported. Nothing here throws: each simulation is guarded individually and the pass as a whole is guarded again, because a diagnostic that fails must still let the session start.
- `submitDriver(script; project=Base.active_project(), options...)` submits the driver script itself as a SLURM job, so the process that records outcomes runs under the scheduler rather than in an SSH session. It writes `data/outputs/drivers/<timestamp>/driver.sh` (body `julia --project=<project> <script>`), submits it with `--parsable`, `--output`/`--error` into that folder and a default `--job-name=mm-driver`, keeps the `sbatch` client's streams as `hpc.out`/`hpc.err` there, prints the job ID with the `squeue -j`/`sacct -j` lines to watch it, and returns the ID. Keywords become `--key=value` flags with underscores turned into hyphens; values must be `String`s or numbers, and the reserved keys are refused. A refusal throws `SubmissionRefused` with `simulation_id === nothing`.
...
- `databaseDiagnostics` on a planted database: a `Running` simulation with a sentinel holding `0` becomes `Completed` and one holding `1` becomes `Failed`; a `Running` simulation with no sentinel but a job ID in `hpc.out` becomes `Completed` when `sacct` reports `COMPLETED`; a `Queued` simulation with no `hpc.out` becomes `Not Started`. The sentinel survives the pass.
- `submitDriver` reaches `sbatch` with `--parsable`, the driver folder's `--output`/`--error`, `--job-name=mm-driver` unless overridden, and the generated script as the final argument; it returns the parsed job ID, and refuses a reserved key or a non-scalar option value with an `ArgumentError`.
...
- `run(T; post_processor=nothing, …)` invokes the post-processor once per **successfully completed** simulation, in the ordering above. Failed/skipped simulations do not trigger it.
- **A validator and a function, not a returned closure.** `run` converts `post_processor` once with `_validatePostProcessor(x) → Vector{QoI}` (a bare function is wrapped into a `QoI`, a `QoI` into a vector, a vector passes through, an empty vector is refused), and `processSimulationTask` — whose keyword is typed `Union{Nothing,Vector{QoI}}` — calls `_postProcess(qs, simulation)` per simulation inside the `:post_processor` stage. `run`'s own keyword is `Union{Nothing,Function,QoI,AbstractVector}`, so a `QoI` is accepted by the signature rather than only by the body. The previous adapter returned a closure, which left the per-simulation body unnameable: `run` held an opaque `Function`, no signature could say what it took, and testing the body meant building the closure first. The `nothing` refusal, the `missing` skip and the anonymous-name refusal stay **per value** rather than moving to validation time, because a side-effects-only anonymous callback that returns `missing` names nothing and is legitimate.
...
- Return-value contract — the SINK's rule, applied to `compute`'s value in `_postProcess` before anything reaches the sink, and not a rule about QoI values in general (sensitivity analysis applies the same one to `reduce`'s value; calibration applies none):
  - `missing` → nothing is stored for that simulation. This is also how a side-effects-only callback says "store nothing".
  - `nothing` → **refused**, with an error naming the QoI and asking for `missing`. It is what a callback returns when a block falls through (a trailing `if` with no `else`, a `for`, a `push!`), so accepting it as "store nothing" would make a dropped measurement indistinguishable from an intended skip.
  - a `Real` → one column named after the QoI. Every value routed through a `QoI` is converted to `Float64` by `_keyedEntries`, so a QoI-written column is REAL whatever numeric type `compute` returned; `_postProcessingColumnSpec` keeps its `Bool`/`Integer` branches for a value written by another route.
  - `NamedTuple` / `AbstractDict` of `name => Real` → one column per key. Either way one row keyed by `simulation_id` is upserted into the sink DB `data/outputs/postprocessing.db`, table `post_processing`. Columns are added on demand; a re-run overwrites the existing row for that `simulation_id`.
  - **Each key becomes the column `"<qoi name>.<key>"`**, not a bare column — the same rule sensitivity analysis uses for a keyed value, so one measurement names its parts identically wherever it is consumed. Two QoIs that both report a `tumor` therefore no longer land in one column. A `Real` return keeps using the QoI's name alone.
  - **A bare anonymous function cannot write to the sink at all.** Its name is a regularised gensym (`anon_9`) that varies between sessions, and since it now prefixes every column the same script would write a fresh, half-empty set of columns each run. Wrapping it (`QoI("counts", f)`) or passing a named function is the fix; returning `missing` is unaffected, since nothing needs naming.
  - any other return type — a `Vector`, a `String`, a nested keyed value, a keyed value with no keys → refused, with an error naming the QoI and the type. **A `String` is no longer storable**: text about a simulation is a tag, so the sink has no TEXT column type.
  - quantity names are quoted safely for SQL (interior `"` doubled); an `AbstractDict` whose keys collide after string conversion (e.g. `1` and `"1"`) → `ArgumentError` from `_keyedEntries`, where the raw keys are still in hand so the message can name both. The sink's own `allunique` over the ordered pairs remains as the guard for two QoIs that produce one column name.
...
- A `missing` return leaves no sink row; a `nothing` return is refused; a NamedTuple/Dict return produces a joinable row.
...
- Unsupported return types (including a `String`) are refused. Because the value checks live in `_postProcess`, which runs inside the per-simulation post-processing stage, `run` surfaces them the way it surfaces any other post-processor failure: as that stage failing for that simulation, wrapping the `ArgumentError`. Dict keys that collide after string conversion are refused there too — by the one spreading function every consumer goes through, while the raw keys are still in hand — so they surface as that same stage error rather than from the sink's own `allunique`, which remains as the guard for a value reaching it by another route.
...
- Tags are written before any simulation is dispatched, so they survive an interrupted run and are queryable while it is in flight. In `runCalibration` they are also *validated* before the run's `calibrations` row and output folder are created, so a malformed key leaves no orphan behind — the same rule the `progress` and `on_monad_failure` keywords follow.
- **A lone `Pair` given to `tags=` is one tag**, exactly as in `tag!`. A `Pair` is iterable, so splatting one into `tag!` turned a single `"key" => "value"` label into two valueless tags; `_asTagCollection` wraps a lone `Pair`, `AbstractString` or `Symbol` before the splat. (Applied in `runCalibration`; `run` and `createTrial` still splat their `tags=` directly.)
...
- **`size(lv)` reports support cardinality, or `-1` when a latent parameter cannot be enumerated.** `GridVariation` checks for that sentinel. The test is `DiscreteUnivariateDistribution` with finite bounds, not finite bounds alone: `Uniform(0,1)` is finitely *bounded* but not finitely *enumerable*, so the grid must keep rejecting it. Cardinality is `length(support(d))`, not the span `maximum - minimum + 1`: the two agree for the contiguous unit-step supports the discrete path builds for itself, and the span over-counts every other level set — three levels `[1, 5, 9]` were sized 9, and the grid walk, which indexes `collect(support(d))`, then ran off the end of the support.
...
- `functions` holds `QoI`s or bare functions; either way each is called once per *simulation* with a `Simulation`, and a monad's replicates are combined by the QoI's `reduce` (the default per-key mean for a bare function).
- **A `QoI` whose value is a `Dict` or `NamedTuple` yields one analysis per key**, labelled `"<qoi name>.<key>"`; a `Real` yields one labelled with the QoI's own name. This is what lets a keyed measurement — the natural shape for "counts per cell type" — serve all three QoI consumers rather than being rewritten once per key for sensitivity analysis alone.
  - **This is where `reduce`'s value is held to a shape, and the only place.** `evaluateFunctionOnSampling` requires a `Real` or a flat `Dict`/`NamedTuple` of them, because each cell of the design needs one number; `compute` may return anything the reducer can turn into that. Every monad must reduce to the *same* keys, since each key's indices need a value from every monad in the design and a hole has no defensible fill. A mismatch is an `ArgumentError` naming both key sets. The reduction loop is wrapped the way `_evaluateParticle` wraps its calls to user code — `@error` naming the QoI and the monad, then `rethrow()` — so a `compute` the default reducer cannot average is reported with both rather than from inside `_qoiMean`.
  - A monad whose reduced value is `missing` — every replicate's `compute` returned `missing`, or its `reduce` did — is refused with a message naming both causes. Calibration can absorb a `missing` by rejecting the particle; a sensitivity index has no cell to put it in.
  - A `Vector` is not a value an index can be computed from — for now. Keys are what this is built on because they make the alignment explicit: a component has a name that can be a label, and two parameter sets can be checked for the same components without anyone deciding what an index means. The error says that rather than merely reporting the type, and says it as a present limitation rather than a closed question — and says that calibration will take the same value as it stands.
  - A score computed **after** averaging (a discrepancy-to-data metric) may be `reduce`'s whole return; carrying it as one more key is what buys the other two consumers, so it is the documented pattern: `compute` reports the raw quantities plus its own per-simulation score, and `reduce` averages the raw quantities and recomputes the score from those means.
  - The sink uses the same rule, so a keyed measurement names its parts identically in both places. The separator is `.` rather than `_` because `_` is legal inside both a QoI name and a key, making `a_b_c` ambiguous between QoI `a` + key `b_c` and QoI `a_b` + key `c`.
  - **A QoI name may not contain a `.`**, refused at construction. The dot is reserved so a label can be read backwards to the QoI that produced it — which is what makes `calculateGSA!`'s name-based skip exact rather than heuristic. Names ModelManager derives itself are unaffected: `_qoiNameFromFunction` regularises to `[A-Za-z_][A-Za-z0-9_]*`.
...
- `calculateGSA!(gsa_sampling, fs; recompute=false)` **skips a measurement whose results are already present**, so adding a quantity to an existing analysis evaluates only the new one. The skip is decided from the QoI's *name*, before any simulation output is read — which is what makes it a saving; a check on labels could not, since a spreading QoI's labels are unknown until its `reduce` has run on a monad. Every label a QoI produces is its name or its name plus `.` and a key, so the name alone answers the question.
- `recompute=true` forces re-evaluation and overwrite. It is required when the measurement itself changed, because that is undetectable: redefining a function's body in place leaves it indistinguishable from the one already evaluated — the same impossibility behind `stored=:never`.
...
**Sensitivity visualization:** `RecipesBase.jl` recipes (no backend dependency) for the three `GSASampling` subtypes, mirroring the calibration recipes in `sensitivity_visualize.jl`. Each emits one series per label in the `results` dict, in sorted order for reproducibility; the series label includes the quantity's own label only when more than one is present. Because `results` is keyed by `String`, there is no label function to dispatch on — the previous `Dict{Union{Function,QoI},...}` had none for `QoI`, so plotting a `QoI`-driven analysis threw a `MethodError` **as soon as it held two entries**. One entry escaped because the sole unguarded call was `sort(...; by=_gsaFunctionLabel)` and `sort` never invokes `by` on a one-element vector, every other call site being behind `multi = length(...) > 1` — which is why the bug survived a test suite whose recipe tests all pass a single bare `Function`. Parameter (x-axis) names come from the `monad_ids_df` columns after the method's bookkeeping columns (`base` for MOAT; `A`,`B` for Sobolʼ; none for RBD).
...
**Behavioral specification:**
- **Every unusable parameter is reported in one error**, by index and name, each with its own reason — including one that is not a variation at all, which used to raise a `MethodError` from inside the internal that classifies them. The message carries no blanket explanation: the one it used to end with said ABC-SMC needs a continuous prior for every parameter, which discrete calibration made false.
- `CalibrationProblem` groups inputs, parameters, observed data, summary statistic, and distance function. The `parameters` field accepts any `AbstractVector{<:AbstractVariation}` — specifically `DistributedVariation`, `CoVariation{DistributedVariation}`, `DiscreteVariation`, `CoVariation{<:DiscreteVariation}`, or `LatentVariation{<:Distribution}` — and converts them internally via `_toCalibrationParameter` to `CalibrationParameter` objects. Each `CalibrationParameter` pairs an `AbstractCalibrationSource` (the original variation, one of `DVSource`/`CVSource`/`LVSource`/`DiscreteSource`/`DiscreteCoSource`) with the derived `LatentVariation{<:Distribution}` used by the ABC-SMC loop. The ABC-SMC algorithm samples CDF values on [0, 1] for each latent dimension; the stored `LatentVariation`'s maps convert those values into concrete target parameter values at simulation time. The `summary_statistic` and `distance` functions are user-supplied and may be simulator-specific.
...
- **A reference monad's variation cannot be overridden, anywhere.** The `AbstractMonad` `CalibrationProblem` constructor takes it from the reference, as `createTrial(method, reference::AbstractMonad, avs; ...)` always has. `run(::GSAMethod, reference::AbstractMonad, avs; ...)` used to accept a `reference_variation_id` and let it win — not by design, but because it is passed before `kwargs...` and Julia lets the rightmost duplicate win — and now raises an `ArgumentError` naming the `InputFolders` form as the place a variation ID is an independent argument.
- **Calibration flows through `run`.** `run(method::ABCSMC, problem::CalibrationProblem)` starts a run and `run(calibration::Calibration[, method])` continues one — including the bare `run(calibration)`, which reloads both the problem and the method from disk and is the shape a session restart uses, alongside `run(::AbstractTrial)` and `run(::GSAMethod, ...)`. Method-first mirrors `run(::GSAMethod, inputs, avs)`, and returning an `ABCResult` mirrors that method returning a `GSASampling`. `run(::Calibration)` is unambiguous against `run(::AbstractTrial)` precisely because `Calibration` was kept out of the containment hierarchy.
- **A resumed run only ever appends generations, so a changed setting takes effect from the next generation.** No setting is refused, because a generation is the unit of change and none can take effect part-way through one. `max_nr_populations` (a cumulative cap), the four stopping criteria, `epsilon_quantile`, `accept_overflow` and `store_rejected` simply apply to new generations. `max_evaluations` is a cumulative cap too — it counts the evaluations the completed generations already made — so a resume continues only when it is raised above that total, and one whose budget is already spent runs nothing and warns, naming the budget and the `resumeCalibration(cal; max_evaluations=N)` that would continue it. `population_size` gives new generations the new size while earlier ones keep theirs — legal, since weights are normalised per generation, but the run ends up heterogeneous. `perturbation_kernel` is refitted from the previous generation each time, so each stays internally consistent. `cdf_grid_k` is resolved once at loop entry, so snapping applies only to new generations and bank reuse differs either side of the resume. `epsilon_schedule` is **indexed by absolute generation**: generation `t` reads `epsilon_schedule[t-1]`, and generation 1 consumes no entry because it has no threshold. An `L`-entry schedule therefore covers generations 2 through `L+1`. Supplying a 5-entry schedule when 5 generations are already complete schedules exactly one new generation — generation 6, using the *last* entry — and generation 7 onward falls back to `epsilon_quantile`. The warning reports the covered range rather than claiming the whole schedule is ignored.
- **Discrete and mixed parameter spaces are calibratable.** A `DiscreteVariation` or `CoVariation{<:DiscreteVariation}` may be passed alongside continuous parameters. It becomes a `DiscreteUniform` over its value indices, so an ABC-SMC particle coordinate is a CDF value in [0, 1] exactly as for a continuous parameter and the quantile does the quantising. The four perturbation kernels therefore need no discrete counterpart: they operate on CDF coordinates and never see a target value, and `_minDiagVar` already floors a variance that collapses when a generation's particles land in one bin. The posterior CSVs and the `:posterior`/`:transition` recipes report the *level*, not the index.
  - What a discrete parameter costs is resolution, not correctness: the sampler can move within a bin with no effect on the simulation. `cdf_grid_k` snapping does not mitigate that and is **not applied** to a discrete coordinate — the dyadic grid is spread evenly over [0, 1] rather than over the level bins, so snapping would give the levels unequal shares of the prior and, whenever `2^k_eff < L`, leave some levels with no grid point at all and therefore unreachable. Generation 1 weights every accepted particle equally, so nothing downstream corrects that skew. The redundant work is collapsed instead by `use_previous=true` when the monad is created (an identical parameterization reuses the existing monad) and, across runs, by the `SimulationBank`, whose box lookup is unaffected: it always tested the raw proposal, never the snapped one.
  - **A single-level discrete parameter is rejected**, alone among discrete inputs. It can never vary, while its latent `DiscreteUniform(1, 1)` still costs a kernel dimension that every proposal draws and none can move — diluting the fitted covariance and adding a coordinate for the bank and the grid to carry. Nothing about the run would say so, since the posterior column is simply constant. The rejection names the alternative: set the value in the reference monad the problem's `reference_variation_id` comes from and leave it out of `parameters`.
  - `parameters.toml` entries for a discrete source record `"values"` — the levels themselves — rather than the internal `DiscreteUniform`, since the levels are what a reader needs to know which values the run could have visited. `_buildDbToDisplayMappingFromTOML` reads `DiscreteSource` and `DiscreteCoSource` alongside their continuous counterparts, whose entry shapes they share; without those branches a discrete parameter had no display-name mapping, and a `:transition` plot built from disk silently dropped its column.
  - Still rejected: a `LatentVariation` whose latent parameters are a raw `Vector{<:Real}`. In the CDF path that branch's latent values are *indices*, a different convention from the one ABC-SMC needs; passing the `DiscreteVariation` itself performs the conversion instead.
  - The `SimulationBank` returns a `DiscreteNonParametric` over the sorted levels for a discrete column, not `nothing` — `nothing` is the bank's internal signal for "this should not happen", which warns and disables reuse.
  - **The bank's interior filter is per latent dimension**: `0 < u < 1` for a continuous prior, whose CDF reaches its bounds only in the limit, and `0 < u <= 1` for a discrete one, whose top level has a CDF of exactly 1. A single test over the whole coordinate vector excluded every monad run at a discrete parameter's top level, costing reuse on a level the sampler proposes as often as any other.
- **`StudySpec` is the model-and-parameters half of a study**, built once and used for sensitivity analysis or calibration: `inputs`, the user's `variations`, `reference_variation_id`, `n_replicates`, `use_previous`. Consumed through `run(::GSAMethod, spec)` and `CalibrationProblem(spec, observed_data, summary_statistic, distance)` — no new named entry points, since `run` already dispatches on the calibration structs.
  - **It holds nothing a single consumer needs.** `observed_data`, `summary_statistic` and `distance` stay on `CalibrationProblem`; `functions` stays on the sensitivity entry point. A sensitivity study has no observed data, and an optional field half the consumers ignore is how a shared abstraction rots. `use_previous` is the one field calibration ignores — it reuses via the `SimulationBank` instead — so it is marked "(sensitivity only)" wherever it appears.
  - **The user's own variations are kept, not normalised.** The reverse conversion is lossy: a `DistributedVariation`'s display name does not survive it, and the generation CSVs are keyed by that name. `ParsedVariations(spec)` derives the normalised form on demand.
  - **The `AbstractMonad` form takes its reference variation from the reference** and accepts no override, matching `createTrial` and `CalibrationProblem`'s monad forms.
  - **A caller keyword beats the spec's value**, because `kwargs...` is forwarded last and Julia gives the rightmost duplicate precedence. Harmless here — a spec's fields are defaults the user set, not an identity carried by an object, which is the distinction that made the same mechanism a bug for a reference monad.
  - **`show` is the whole reporting surface**, deliberately: no validation function. It prints one line per parameter with its kind and whether sensitivity and calibration can use it, and flags a latent variation without `inverse_maps` — accepted by calibration but silently disabling the `SimulationBank`.
- **`QoI(name, compute; reduce, stored, skip_missing, data)` is the seam between a user's measurement and the three things that consume one.** `compute` is called with one `Simulation`; `reduce` combines a parameter set's replicate values. The `QoI` is passed *directly* to `run(::GSAMethod, ...; functions=)`, to `CalibrationProblem`'s `summary_statistic`, and to `run(...; post_processor=)` — there are no adapter functions in the public API.
  - **There is no level to declare, because per-simulation `compute` plus `reduce` is strictly more expressive than a monad-level `compute`.** A monad-level measurement is that pair with the reduction folded into `compute`, so it added a granularity question — and the `Simulation`-versus-`Monad` dispatch collision with it — for no expressive gain.
  - **Value constraints belong to the consumer that needs them; the seam imposes none.** `compute` and `reduce` are the user's functions and ModelManager does not interpret their values. `compute` returns one simulation's value or `missing`; `reduce` receives the surviving replicate values and returns the parameter set's value or `missing`; `nothing` is refused on both paths, and that is the whole of the seam's rule. Each consumer then applies its own, where it reads the value:
    - **The sink** sees only `compute`'s value and requires a `Real`, or a flat non-empty `Dict`/`NamedTuple` of `Real`s, because each component becomes a column. No `Vector`, no nested keyed value, no struct, no `String` (text about a simulation is a tag). Checked in `_postProcess`, via `_qoiValueShape`.
    - **Sensitivity analysis** sees only `reduce`'s value and requires the same, because each key becomes an analysis needing one number per monad. Checked in `evaluateFunctionOnSampling`, via the same `_qoiValueShape`.
    - **Calibration requires nothing of either.** `reduce`'s value goes into the `SummaryValues` `distance` receives: a `Real` under `(name, nothing)`, a keyed value one entry per key, anything else — a `Vector`, a `Matrix`, a user's struct — whole under `(name, nothing)`. The user's `distance` is the only reader, so it is the only thing that can decide what is comparable.
    - Errors name the QoI, the offending type, the function that produced it, and *whose* rule was broken — a user told "a value must be a `Real`" without that qualification rewrites a `compute` their `distance` would have taken unchanged.
    - The earlier one-rule-for-everyone version constrained things that share no reader: `compute`'s value has one consumer (the sink) and `reduce`'s has two others (GSA, calibration). It also refused values before the function written to handle them ran — a struct-returning `compute` whose `reduce` turns it into a keyed series was rejected at the seam, although calibrating that series against data is the point of writing it.
  - **`reduce` need not return the shape it was given.** A keyed `compute` may reduce to one number and a `Real` one to a keyed value. Tying the two constrained nothing that shares a reader, and the recommended workaround produced the very "same name, two quantities" it was meant to prevent: a score carried as a key is a per-simulation score at the sink and a score-of-means in GSA. Carrying the score as one more key is still the right thing when you *also* want GSA on it — one QoI then serves all three consumers — but it is advice rather than a rule.
  - **Replicates must agree about their keys only for the DEFAULT reducer**, which averages key by key and so has nowhere to put a key one replicate lacks. Enforced inside `_qoiMean`, not at the seam: enforcing it before any `reduce` ran meant a reducer written to reconcile ragged replicates — filling an absent cell type with a zero, PCMM's `endpointPopulationCountQoI` being a real example — never saw them. Keys are compared as strings, as they are everywhere else, so a `Dict` and a `NamedTuple` naming the same quantities agree. The message says whose rule it is and points at `reduce=`.
  - **The default `reduce` is a keyed-aware mean:** `mean` for `Real`s, and the per-key mean for keyed values, returned in the first replicate's kind of container and under its own keys. `mean` itself cannot combine two `Dict`s, so before this every keyed QoI needed a hand-rolled reducer and a bare keyed function died inside `Statistics.mean` naming no QoI. **Components need not be `Real`** — a `Dict(key => Vector)` averages elementwise, which is the ordinary shape of a time series per cell type — so the per-key `mean` is guarded instead, naming the key and the component type where a nested keyed value would otherwise die as a bare `MethodError`.
  - **`compute` returns `missing` to say "no value for this simulation".** With `skip_missing=true` (the default) missing replicates are dropped before `reduce` — via `collect(skipmissing(...))`, so the element type narrows — and a parameter set with none left is itself `missing`. `skip_missing=false` hands `reduce` the raw vector, missings included. **`reduce` may itself return `missing`**, which is the same answer one level up — "the replicates I got are not enough to answer with" — and is a supported return rather than an accident. `nothing` is refused on both paths, with a message asking for `missing`.
  - **A plain `Function` is wrapped into a `QoI` at the boundary**, gaining the two things it lacks: a name (from `_qoiNameFromFunction`, so an anonymous one becomes `anon_…`) and the default reducer. Nothing downstream branches on which it was given.
  - **A keyed value is NAMED by one rule for all three consumers**, in `_qoiComponentLabels`: each component is `(qoi name, component key)`, the component key being `string(k)`, or `nothing` for a `Real`-valued QoI, and two keys of one value that stringify alike are refused where the raw keys are still in hand. Two spreaders share it: `_keyedEntries`, which produces `Float64`s for the sink and GSA (whose `Dict{Int,Vector{Pair{_SummaryKey,Float64}}}` depends on the element type) and refuses anything else, and `_summaryEntries`, which produces `Any` for calibration and refuses nothing — an unkeyed value of any type is one entry under `(name, nothing)`, and an empty keyed value contributes none (routing it to the scalar slot would make the summary's key space depend on the value). `summaryLabel` turns that pair into the sink column and the sensitivity label, `"<qoi name>.<key>"`; the dot rather than an underscore because `_` is legal inside both a QoI name and a key, making `a_b_c` ambiguous between QoI `a` + key `b_c` and QoI `a_b` + key `c`. **Calibration uses the pair itself** — see the calibration feature below. Before this the three consumers each spread a value with their own code and agreed only by inspection.
  - **QoI names need not be unique; the `(name, key)` pair must be.** Two keyed QoIs sharing a name with disjoint component keys is not pathological, so neither `_validateSummaryStatistic` nor the post-processor validator checks names. What each consumer checks is its own collision: the `_SummaryKey` for calibration, the column name for the sink (`_normalizePostProcessingQoI`'s `allunique`), and the label for GSA (`calculateGSA!`'s cross-QoI check). Two `Real`-valued QoIs with one name collide under all three, since their key is `(name, nothing)` both times.
  - **`data` is an optional slot for whatever the measurement needs besides the simulation** — an observation to score against, a set of cell types. It changes the calling convention explicitly and without method sniffing: when `data !== nothing`, `compute(simulation, data)` and `reduce(values, data)`; otherwise one argument each. Sniffing was rejected because a `compute` that merely happens to accept two arguments would be called with data it was never written for, and the failure would surface inside the user's own function rather than at the call site. `data` is serialised inside the `QoI` in `problem.jld2`, so a calibration resumes with it and `_ProblemManifest` needed no field for it; restorability is still decided by the two functions (`_isAnonymousFunction`), which is why the manual's post-averaging example uses `data=` rather than a closure or a callable struct.
  - **`missing` at each consumer:** the sink stores nothing for that simulation; sensitivity analysis refuses a QoI whose value is `missing` for any monad, saying that monad has no value because every replicate's `compute` returned `missing` or its `reduce` did — both causes, since the code cannot tell which it was; calibration treats a `missing` summary the way `on_monad_failure` treats a monad with no successful simulation.
- **A `QoI` can read a value the post-processing sink stored earlier**, via `stored=:prefer` (stored if present, else compute) or `stored=:require` (stored or error). A keyed QoI is reassembled from the `"<name>.<key>"` columns the sink wrote, as a `Dict` with `String` keys, so `stored` works for a keyed measurement and not only a scalar one; `:require`'s error names those columns. This is the write-once-read-later path that matters once post-simulation cleanup has deleted whatever the quantity was computed from.
  - **A closure is not restorable, however it was written.** `_isAnonymousFunction` means "cannot be restored by name in a fresh session": true for a lambda and for a named function defined inside another function, a `let` or a `@testset`; false for a top-level function and for a callable struct. A closure's derived QoI name comes from its type and is an `anon_…` form, which the sink refuses to store under and which sensitivity analysis never uses to skip work, because two closures from one factory share it. A top-level name is kept in any alphabet.
  - **`resumeCalibration` reaches `problem=` even when `problem.jld2` cannot be read back**: the manifest is loaded with `required = isnothing(problem)`, so an unreadable file is an error naming the two ways out (re-`include` the defining file, or pass `problem=`) when nothing was supplied, and a warning plus the supplied problem, unvalidated, otherwise.
  - **`stored` defaults to `:never`, and the reason is a measured impossibility rather than caution.** Nothing records which `compute` produced a stored value, and no fingerprint can: redefining a function's body in place leaves both `hash` and `nameof` unchanged, so a changed `compute` is undetectable, while two textually identical anonymous functions hash *differently*, so an unchanged one is equally unrecognisable. A fingerprint fails in both directions. The sink carries no provenance either — the table is `simulation_id` plus one column per name.
  - **`verifyStoredValues(q, T)` is the robust check, because it recomputes rather than fingerprints.** It reports agreements, mismatches (with the offending simulation IDs and both values), how many were never stored, and how many are *unverifiable* because the output folder is gone — which is exactly the case `stored` exists for. Ground truth where it is available, and an honest "cannot tell" where it is not.
- **`run_kwargs` and a loose keyword splat are one channel, accepted everywhere.** Calibration has to bundle — `runABC` and `resumeCalibration` spend their splat on `ABCSMC` field forwarding, leaving none for simulator options — while `run` has always taken loose keywords. `run(::AbstractTrial)`, `run(::AbstractVector)` and `run(::AddVariationMethod, ...)` now accept `run_kwargs::NamedTuple` too, merged through `_mergeRunKwargs`, so a bundle assembled once is portable to any entry point. **Where a key appears both ways the loose keyword wins**, being the more deliberate spelling at the call site.
  - **`runCalibration` deliberately does *not* gain a loose splat.** It takes the method positionally, so a splat was available — but adding one would turn its current `MethodError` on a mistyped keyword into a silent forward to the simulator hooks, which accept anything. The convention is additive in the direction that does not lose error checking.
  - **A `run_kwargs` bundle can no longer hijack calibration's own run controls.** It was splatted *last* into the per-batch `run`, so `run_kwargs=(quiet=false,)` or `(on_progress=f,)` silently replaced the progress machinery the `progress=` keyword had just configured. The calibration's own controls now come after it.
...
- `runABC(problem; kwargs...)` and `runCalibration(method, problem; ...)` run the full SMC loop. **`runCalibration` takes the method first**, matching `run(::GSAMethod, inputs, avs)` and `run(::ABCSMC, problem)`; `resumeCalibration(calibration[, method])` leads with the object being resumed, there being nothing else to lead with. Breaking, and taken while pre-1.0.
...
- **`distance` always receives a `SummaryValues`, keyed by the same `(qoi name, component key)` pairs every other consumer uses.** `SummaryValues <: AbstractDict{Tuple{String,Union{Nothing,String}},Any}` is exported, holds a `Dict` plus a `Vector` recording insertion order (deterministic iteration and messages, with no OrderedCollections dependency), and implements `length`, `iterate`, `keys`, `values`, `haskey`, `getindex`, `get` and `show` (as `label => value`, each value rendered under `:compact`/`:limit` so a 10,000-point series does not print in full inside an error message). **The `Any` is load-bearing:** a `Float64` element type forced every `reduce` to produce numbers before `distance` — the reader that decides — had seen anything, so a time series could not be calibrated against a time series.
  - **Flexible lookup, in one order.** `s[k]` for a `String` (or a `Symbol`, stringified) tries: `(k, nothing)`, a `Real`-valued QoI named `k`; then, if `k` contains a `.`, a split at the FIRST one into `(name, key)` — a QoI name cannot contain a dot, so `"counts.a.b"` is unambiguous; then every entry whose component key equals `k`, which resolves only when exactly one QoI produced it. Several is an `ArgumentError` listing the qualified labels to choose between; none is an `ArgumentError` listing the labels that exist. A `Tuple` key is exact. `haskey` mirrors `getindex`, so an ambiguous bare key is `false` rather than a throw.
  - **Always a `SummaryValues`**, for a single `QoI` as much as a vector and for a `Real`-valued QoI as much as a keyed one. The single-QoI passthrough that handed a bare `Real` over made `q` and `[q]` two contracts for one measurement; a scalar `observed_data` stays usable through `mseDistance(::SummaryValues, ::Any)` instead, which requires the summary to hold exactly one value — the condition a bare number actually implies.
  - **The pair is what makes this one namespace rather than three.** Keying `distance`'s argument by the user's bare component names made a QoI used for calibration narrower than the same QoI used for the sink or GSA, and forbade two keyed QoIs sharing a component key — a `count` and a `speed`, both per cell type. `A Vector{QoI}` used to *nest* into `Dict(name => Dict(...))`, for which `mseDistance` had no method at all.
- A summary that is `missing` — some member has no value, because every replicate's `compute` returned `missing` or its `reduce` did — makes the whole summary `missing`, and the particle follows `on_monad_failure` rather than being reported as a fault in the user's functions. Nothing failed on this path, so no simulation or monad ID reaches the generation's failure files; instead the monads it happened to are named in a **per-generation warning** of their own (count, compressed IDs, and that their particles were rejected), once per generation and silent at `progress=:none`, so a measurement that never has a value cannot be mistaken for a model that never fits. Under `:error` the message names the monad and the QoI that had no value, rather than failure files that were never written.
- `mseDistance(simulated, observed)` is a family of built-in distance functions. **The observed keys are the comparison; extra simulated components are ignored**, since a simulation is always known better than the data — requiring the observation to name every component forced a user to list every key of every summary QoI in `observed_data` or drop QoIs from the summary. Nothing is zero-filled in the other direction: an observed key that does not resolve is still an error. **Normalisation is one global mean:** the squared differences are summed and divided by how many differences were computed, so a single array-valued key gives exactly `mean(abs2, sim .- obs)`, `mseDistance(v1, v2)` standalone agrees with the same arrays inside a one-key summary, and for all-scalar keys the number is unchanged. The documented consequence is that a 100-point series contributes 100 differences where a scalar key contributes one. One helper, `_mseTerms(sim, obs[, label]) -> (sum of squares, count)`, does every comparison: `(Real, Real)` directly, anything else through `sum(abs2, sim .- obs)` in a try/catch whose message names both types and, when both sides have a `length`, both lengths — a 101-point observation against a 100-point simulation being the commonest array mistake.
  - `mseDistance(::SummaryValues, ::Union{AbstractDict,NamedTuple})` — iterates the OBSERVED keys, each resolved through `SummaryValues`' own lookup, so it may be qualified (`"counts.tumor"`), bare (`"tumor"`, when one QoI reports it), a `Real`-valued QoI's name, or an exact tuple. An unresolvable or ambiguous key raises that lookup's error. Three refusals guard the doors opened by dropping strictness: an observation that **names nothing** (with no terms the mean is 0/0, and returning 0.0 would make every particle perfect and hand ABC-SMC back the prior — the zero-fill failure re-entering by the other door), **two spellings resolving to one entry** (naming both; it would be compared twice and weighted double), and a resolved value that is **`missing`** (naming the label, rather than surfacing a frame later as "`distance` returned a Missing").
  - `mseDistance(::SummaryValues, ::Any)` — the summary must hold exactly one value; otherwise an `ArgumentError` lists what it holds, since which of several a bare observation meant cannot be guessed. `::Any` rather than `::Real` so a one-entry summary holding an array works against an array observation — and because `Base.broadcastable(::AbstractDict)` throws, so a `SummaryValues` must never reach the generic fallback.
  - `mseDistance(::Union{AbstractDict,NamedTuple}, ::Union{AbstractDict,NamedTuple})` — the generic keyed-vs-keyed method, for a comparison outside calibration. Observed ⊆ simulated by `string(k)`, so a `Dict` may be compared with a `NamedTuple` and `:a` with `"a"`; an observed key the simulated value does not report is an `ArgumentError` listing each side's RAW keys with `repr`, so a genuine `Symbol`-versus-`String` difference is visible rather than printed as two identical lists. Two empty keyed values still give 0.0 here: this is a script utility, not the thing an ABC-SMC run's acceptance depends on.
  - `mseDistance(::Real, ::Real)` — squared difference `(sim - obs)^2`.
  - `mseDistance(::Any, ::Any)` — the generic fallback, through `_mseTerms`: two arrays, or anything else that broadcasts. It is what makes a QoI reducing to a time series calibratable with the built-in distance and no PCMM change.
- `CalibrationProblem.observed_data` is typed `Any`; any type accepted by the user-supplied `distance` function is valid.
- `summary_statistic` is a `QoI` or a vector of them (a bare function is wrapped into one); calibration places no constraint on its value, which reaches `distance` inside a `SummaryValues` exactly as `reduce` returned it.
...
- **`show(::Calibration)` names the epsilon it prints.** A run records two — the threshold a generation was run against and the largest distance it accepted — and this is the second, labelled `Max ε accepted`. It was the one user-facing place where the bare label `Final ε` left the reader to guess which.
...
- **`delete_subs=true` deletes the monads only the named runs used**, mirroring `deleteSampling`'s guard. `_unsharedCalibrationMonadIDs` drops from the deletion list every monad that appears in the constituents of another sampling or in another calibration's `calibrationMonadIDs`. The mirror of `deleteSampling` is only correct with one correction: each run's own per-generation batch samplings list every monad it evaluated, so they are excluded from the "other samplings" guard, identified by the `mm:calibration` tag `_buildEvaluateBatch` stamps on them (`_batchSamplingIDs`). Sharing is the normal case, not an edge case, because the bank is keyed on inputs and simulator version rather than on the calibration. `deleteMonad` keeps `delete_supers=true`: once the guard has run, the only samplings still holding a deleted monad are the runs' own batches, so the upward cascade rewrites those to their surviving monads, removes the empty ones, and reaches nothing else. (`deleteSampling` can pass `false` only because it has already removed its own rows; a surviving row that lists deleted monads makes `Sampling(id)` throw.)
...
- **Failed simulations during calibration.** After each batch runs, `evaluate_batch` classifies its simulations via `_batchOutcome`, which compares the pre-run snapshot of every monad's simulation IDs against their current `status_code_id` (one `_simulationStatusIDs` query per batch) and returns three sorted ID vectors: simulations now marked `Failed`, monads with at least one such simulation, and monads with **no** `Completed` simulation. The snapshot must be taken *before* the batch runs: when every simulation in a monad fails, the runner marks each `Failed`, erases it from the monad's constituent list, and deletes the emptied monad (`simulationFailed` → `eraseSimulationIDFromConstituents` → `deleteMonad`), taking with it the constituent CSV that is the only monad→simulation mapping (the `simulations` table stores input/variation IDs, not monad membership).
  ...
  - **The same policy covers a monad whose summary statistic is `missing`** — it has output, but every replicate's `compute` declined to produce a value, so there is equally nothing to compare. `_evaluateParticle` returns `missing` under `:reject` and raises under `:error`, naming the monad. It is deliberately *not* routed to the user-code fail-fast path below: returning `missing` is the supported way for a measurement to say "not for this simulation", not a bug.
...
- **Posterior visualization** (`task #7`): Implemented via `RecipesBase.jl` (no backend dependency; works with any Plots.jl- or Makie-compatible backend). Four recipe types and two accessor functions:
  ...
  - **Proposal distances** — `plot(result, :distances)` or `plot(Calibration(id), :distances)`. Histogram of every proposal a generation evaluated, with the accepted ones coloured separately and the threshold drawn on a bin boundary. Bin edges are computed by the recipe, not the backend: the rejected distances extend well past the accepted ones, so per-series auto-binning would misalign the very boundary the plot is about. Accepted and rejected bars are provably disjoint — acceptance is `distance <= ε`, so a distance equal to ε is constrained to the last accepted bin rather than the first rejected one. `logscale=true` bins in log10 and drops non-positive distances, reporting how many. **Non-finite distances are dropped and counted in the title** the same way: `Inf` is an ordinary return from a user's `distance` for a hopeless parameter set, and it has no bin — clamping it onto the last bar would draw a distance the run never measured. Bin widths stay uniform, since the recipe passes one `bar_width` for every bar; the leftmost edge can therefore sit up to a bin below the smallest distance, and it is the x-axis default that starts at the data, so a squared-error histogram does not open on a negative axis. The generation is addressed by position in the in-memory result and bounds-checked, and the title reports the generation's own `t` — after a resume that skipped an unreadable generation the two differ.
...
- **CDF-grid snapping with simulation bank lookup** (`task #17`, implemented): All computation happens in CDF (latent coordinate) space. Every generation follows the same procedure; only the proposal source differs (prior for t=1, kernel perturbation of previous particles for t>1).
  ...
  *Per-generation procedure:*
  ...
  3. **Fallback snap.** Only if the bank lookup yields no usable candidate: snap θ_prop to the nearest interior grid point g* at resolution k_eff, in each dimension whose latent prior is continuous. A dimension whose prior is discrete is left at its proposed CDF value — the grid does not divide evenly into its levels, and the quantile already collapses each level's whole bin to one target value. If that grid point was already evaluated this generation, discard the proposal. Otherwise use g* as the effective proposal (a new simulation is launched at the grid point).
...
- `mseDistance` returns 0.0 when simulated equals observed (both calling conventions).
- `mseDistance(3.0, 1.0) == 4.0`, and `mseDistance(Dict("a" => 3.0), Dict("a" => 1.0)) ≈ 4.0`.
- `mseDistance` on two dicts throws an `ArgumentError` listing both key sets when the OBSERVED value names a key the simulated one does not; a simulated key the observation does not name is ignored. `mseDistance([1.0, 2.0], [3.0, 4.0]) ≈ 4.0` through the generic fallback, and a length mismatch is an `ArgumentError` naming both lengths.
...
- `deleteCalibration` removes the row, its tags, and its folder while leaving the monads and simulations intact; with `delete_subs=true` it removes the monads only that run used, and their simulations.
- Of two calibrations sharing monads through the bank, `deleteCalibration(first; delete_subs=true)` leaves every monad the second also evaluated, leaves one held by a hand-built `Sampling`, and deletes the rest with their simulations.
