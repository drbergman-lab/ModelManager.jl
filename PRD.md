...
- Simulator packages call `registerSimulator!(MySimulator(...))` in their `__init__`; it takes nothing but the simulator, and a backend that wants a concurrency default calls `setNumberOfParallelSims` on the next line, which needs only registration. It creates the globals when none exist, returns the existing ones untouched when the same backend type is already registered, and warns naming both types when it replaces a different backend's. One process serves one backend.
