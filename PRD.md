...
- Return-value contract — the QoI value contract, enforced at `_asPostProcessor` before anything reaches the sink:
  - `missing` → nothing is stored for that simulation. This is also how a side-effects-only callback says "store nothing".
  - `nothing` → **`ArgumentError`**, naming the QoI and asking for `missing`. It is what a callback returns when a block falls through (a trailing `if` with no `else`, a `for`, a `push!`), so accepting it as "store nothing" would make a dropped measurement indistinguishable from an intended skip.
  ...
  - any other return type — a `Vector`, a `String`, a nested keyed value → `ArgumentError` naming the QoI and the type. **A `String` is no longer storable**: text about a simulation is a tag, so the sink has no TEXT column type.
...
- A `missing` return leaves no sink row; a `nothing` return is an `ArgumentError`; a NamedTuple/Dict return produces a joinable row.
...
- Unsupported return types (including a `String`), and dict keys that collide after string conversion, raise `ArgumentError`.
...
- **A `QoI` whose value is a `Dict` or `NamedTuple` yields one analysis per key**, labelled `"<qoi name>.<key>"`; a `Real` yields one labelled with the QoI's own name. This is what lets a keyed measurement — the natural shape for "counts per cell type" — serve all three QoI consumers rather than being rewritten once per key for sensitivity analysis alone.
  - Every monad must reduce to the *same* keys, since each key's indices need a value from every monad in the design and a hole has no defensible fill. A mismatch is an `ArgumentError` naming both key sets.
  - A monad whose reduced value is `missing` — no replicate produced a value — is refused with a message saying so. Calibration can absorb a `missing` by rejecting the particle; a sensitivity index has no cell to put it in.
  - A `Vector` is not a QoI value at all, for the same reason it could not be spread by index: only its length can be checked across monads, and equal length is not equal meaning (two series sampled at different times share a length and not a meaning). The error says so rather than merely reporting the type.
  - A score computed **after** averaging (a discrepancy-to-data metric) travels as one more key rather than as `reduce`'s whole return, since `reduce` must keep its shape: `compute` reports the raw quantities plus its own per-simulation score, and `reduce` averages the raw quantities and recomputes the score from those means.
...
- **`QoI(name, compute; reduce, stored, skip_missing)` is the seam between a user's measurement and the three things that consume one.** `compute` is called with one `Simulation`; `reduce` combines a parameter set's replicate values. The `QoI` is passed *directly* to `run(::GSAMethod, ...; functions=)`, to `CalibrationProblem`'s `summary_statistic`, and to `run(...; post_processor=)` — there are no adapter functions in the public API.
  ...
  - **One value contract, everywhere.** A QoI value — what `compute` returns per simulation and what `reduce` returns per parameter set — is a `Real`, or a flat `Dict`/`NamedTuple` whose values are all `Real`. No `Vector`, no nested keyed value, no struct, and no `String` (text about a simulation is a tag). It is enforced at two seams: `compute`'s value in `_asPostProcessor` and in `_reduceOverMonad`, and `reduce`'s value in `_reduceOverMonad`. Sensitivity analysis keeps its per-component `Real` check as a second line. Errors name the QoI and the offending type.
  - **`reduce` is shape-preserving:** a `Real` per simulation reduces to a `Real`; a keyed value reduces to the same key set. Checked after `reduce`. A consequence is that a score computed *after* averaging travels as one more key rather than as the whole return.
  - **Replicates must agree about their keys**, checked in `_reduceOverMonad` before `reduce` is called. It is the default reducer's requirement, but it lives there because that is the only place that knows which QoI is being evaluated — and because the shape check above is undefined when the replicates disagree about what the keys are.
  - **The default `reduce` is a keyed-aware mean:** `mean` for `Real`s, and the per-key mean for keyed values, returned in the same kind of container. `mean` itself cannot combine two `Dict`s, so before this every keyed QoI needed a hand-rolled reducer and a bare keyed function died inside `Statistics.mean` naming no QoI.
  - **`compute` returns `missing` to say "no value for this simulation".** With `skip_missing=true` (the default) missing replicates are dropped before `reduce` — via `collect(skipmissing(...))`, so the element type narrows — and a parameter set with none left is itself `missing`. `skip_missing=false` hands `reduce` the raw vector, missings included. `nothing` is refused on both paths, with a message asking for `missing`.
  ...
  - **A keyed value is spread by both the sink and sensitivity analysis, under one naming rule: `"<qoi name>.<key>"`.** The dot rather than an underscore because `_` is legal inside both a QoI name and a key, making `a_b_c` ambiguous between QoI `a` + key `b_c` and QoI `a_b` + key `c`. **Calibration prefixes nothing** — see the calibration feature below.
  - **`missing` at each consumer:** the sink stores nothing for that simulation; sensitivity analysis refuses a QoI whose value is `missing` for any monad, saying that monad had no replicate that produced a value; calibration treats a `missing` summary the way `on_monad_failure` treats a monad with no successful simulation.
...
- **The keys `distance` sees are the keys the user's `compute`/`reduce` produced.** A single `QoI` passes its value through unwrapped, so a keyed one hands over its own `Dict`/`NamedTuple` keys. A `Vector{QoI}` flattens into one `Dict{String,Float64}` — a `Real`-valued member contributes its own name, a keyed member contributes its keys as written — and a key claimed by two QoIs is an `ArgumentError` naming both, raised at the first evaluation where it happens. Nothing is prefixed with a QoI's name here, unlike sink columns and sensitivity labels: those are flat namespaces shared across a project, while `distance`'s two arguments are private to one problem and the user writes the other one. A `Vector{QoI}` used to *nest* into `Dict(name => Dict(...))`, for which `mseDistance` has no method at all.
- A summary that is `missing` — every replicate of some member returned `missing` — makes the whole summary `missing`, and the particle follows `on_monad_failure` rather than being reported as a fault in the user's functions.
- `mseDistance(simulated, observed)` is a family of built-in distance functions:
  - `mseDistance(::AbstractDict, ::AbstractDict)` — mean of the per-key squared errors. The two must carry **exactly** the same keys; a mismatch is an `ArgumentError` listing what each side has. It previously warned once and imputed an absent key as zero, which turned a naming mistake into a converged, wrong posterior: comparing every key against 0 is a perfectly finite distance, so ABC-SMC accepted particles on it.
  - `mseDistance(::Real, ::Real)` — squared difference `(sim − obs)²`.
  - There is no `AbstractVector` method: under the QoI value contract no summary can be a `Vector`.
- `CalibrationProblem.observed_data` is typed `Any`; any type accepted by the user-supplied `distance` function is valid.
- `summary_statistic` is a `QoI` or a vector of them (a bare function is wrapped into one); its value follows the QoI value contract above.
...
- `mseDistance` on two dicts with different key sets throws an `ArgumentError` listing both, in either direction; there is no `AbstractVector` method to call.
