...
- `packageName(sim)` defaults to the package defining `typeof(sim)`, resolved through `Base.moduleroot` so a type in a submodule reports its package. It is the **only** interface method deciding which package is version-checked, and the only one a backend overrides for this: `getInstalledVersion` looks that name up in the environment, and the internal `_loadedPackageVersion` looks it up in `Base.loaded_modules`, so the two always describe the same package and overriding `packageName` moves both. The installed version is never substituted for the loaded one.
- `_loadedPackageVersion` is internal, not an interface hook: any value other than the named package's real loaded version would break the installed-vs-loaded comparison the design rests on, so there is no override a backend could usefully supply. It yields `nothing` when the named package is not loaded, carries no version, or is ambiguous — a name matching two loaded packages resolves to `nothing` rather than to whichever `Dict` iteration reaches first.
...
- Every version diagnostic is emitted from one place, at a consistent level: `@warn` when the loaded version lags the installed one (`maxlog=1`, since the loaded version is fixed for a session) and when the session lags the database; `@error` when the database is ahead of the installed version, and when a migration target is beyond the loaded version; `@info` for migration progress. Each names a mid-session `Pkg` change as the cause. `continueMilestoneUpgrade` remains a `println`, since a `readline` follows it.
