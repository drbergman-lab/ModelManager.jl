...
**Sensitivity visualization:** `RecipesBase.jl` recipes (no backend dependency) for the three `GSASampling` subtypes, mirroring the calibration recipes in `sensitivity_visualize.jl`. Each emits one series per label in the `results` dict, in sorted order for reproducibility; the series label includes the quantity's own label only when more than one is present. Because `results` is keyed by `String`, there is no label function to dispatch on — the previous `Dict{Union{Function,QoI},...}` had none for `QoI`, so plotting a `QoI`-driven analysis threw a `MethodError` **as soon as it held two entries**. One entry escaped because the sole unguarded call was `sort(...; by=_gsaFunctionLabel)` and `sort` never invokes `by` on a one-element vector, every other call site being behind `multi = length(...) > 1` — which is why the bug survived a test suite whose recipe tests all pass a single bare `Function`. Parameter (x-axis) names come from the `monad_ids_df` columns after the method's bookkeeping columns (`base` for MOAT; `A`,`B` for Sobolʼ; none for RBD).
  ...
  - **Every GSA recipe takes `parameters`** (read with `pop!(plotattributes, :parameters, nothing)` like `show_sigma`/`show_ST`), the same selector vocabulary as the calibration recipes, resolved against the x-tick names by `_selectParameters`. The builders take it as a keyword and slice `means_star`, `variances`, `elementary_effects[:, idx]`, `S1` and `ST` to the selected positions, in the selected order, so the bar, violin and scatter charts all agree on which parameters they show.
...
- `parameters` on a builder returns only the selected parameters, in the requested order, with index vectors and the elementary-effect matrix sliced consistently; a `Regex` and `Not` resolve; an unknown name throws an `ArgumentError` naming the available parameters.
...
- **Posterior visualization** (`task #7`): Implemented via `RecipesBase.jl` (no backend dependency; works with any Plots.jl- or Makie-compatible backend). Four recipe types and two accessor functions:
  ...
  - **Any recipe with a parameter axis takes `parameters`** — the corner plot, `:ridgeline` and `:transition`, on both `ABCResult` and `Calibration`. `nothing` (default) draws every parameter; otherwise it is a DataFrames column selector over the names the recipe would have drawn — a name, a vector of names, positions, a `Regex`, `Not(...)` — resolved by the shared `_selectParameters` (`sensitivity_visualize.jl`). The names are the ones the user already sees: the columns of `posterior(result)` in `space=:target`, the latent names in `space=:cdf`. A vector of names is honoured in the order given, which is how panels are reordered; an unknown name is an `ArgumentError` listing what was available. `:distances` has no parameter axis, so passing `parameters` to it throws rather than being swallowed by RecipesBase's keyword handling. Filtering happens in the top-level recipes after the space is resolved, before the `_CornerPlotData`/`_RidgelineData`/`_TransitionData` wrapper is built, so the wrappers and their recipes are unchanged.
