...
- **Failed particle evaluation.** `summary_statistic` and `distance` are user code run against a monad whose simulations may have failed. When *every* simulation in a monad fails, the runner marks each `Failed`, erases it from the monad's constituent list, and deletes the emptied monad (`simulationFailed` → `eraseSimulationIDFromConstituents` → `deleteMonad`), after which the user's statistic typically throws (`Monad N not in the database`) or returns `missing` — which then throws inside `distance`. `runABC`, `runCalibration`, and `resumeABC` accept `on_evaluation_failure::Symbol=:reject` (validated up front; `ArgumentError` on anything but `:reject`/`:error`):
  - `:reject` (default) — the particle's distance becomes `Inf`, so ABC-SMC rejects it and the run continues. A warning names the monad, the exception, and the diagnosis: for a monad no longer in the database, that all of its simulations failed, listing their output folders; for a monad still present, that the fault is more likely missing simulation output or a bug in the user's `summary_statistic`/`distance`, with a pointer to `:error`.
  - `:error` — logs the same calibration context via `@error` and then rethrows, preserving the original backtrace (deliberately *not* wrapped in a new exception).

  Monad construction from a `known_mid` is guarded identically, so a stale bank entry rejects rather than aborting the run. Each monad's simulation IDs are snapshotted *before* the batch runs, because a deleted monad takes its constituent CSV — the only pointer to those output folders — with it.

  Rejection warnings are throttled to the first `_MAX_REJECTION_WARNINGS` (5) per generation; the per-generation total is reported by `_logGenerationRejections` from the `on_generation` callback once the generation is saved. All failure reporting is silent at `progress=:none` and is gated to `:generation` and above. Additionally, because calibration runs each batch with `quiet=true` (suppressing the runner's own low-success notice), `evaluate_batch` inspects the returned `MMOutput` and warns when `n_success < n_scheduled`.

  `_buildEvaluateBatch` returns `(evaluate_batch, rejection_counts::Dict{Int,Int})` — the tally is owned by the closure and read by `on_generation`.
- **Non-finite distances in generation 1.** Generation 1 has no epsilon threshold and accepts every proposal, setting `epsilon = maximum(distances)`. A single `Inf` would therefore make ε infinite, after which every later proposal passes `distance <= epsilon` and the run silently degenerates. `_acceptFirstGeneration` drops non-finite-distance proposals before the result is built, warns how many were dropped, and renormalizes the uniform weights over the survivors; `n_evaluations` still counts every proposal while `n_accepted` counts only survivors, so the acceptance rate stays honest. It errors when *no* particle survives — a whole generation of failed evaluations is a broken model, not sampling noise. Later generations need no such handling: `Inf` fails `distance <= epsilon` on its own, and a generation whose batches all fail keeps proposing (bounded by `max_evaluations`) rather than aborting, so one bad monad in a small batch cannot kill a run.
- **Rejected monads are never banked.** `_updateMidGenAdditions!` skips proposals with a non-finite distance. A monad whose every simulation failed has been deleted, so banking its ID would hand it back as a `known_mid` reuse in a later generation, where reconstructing it fails — the same error the rejection handling exists to absorb, but raised inside ModelManager rather than user code.
