...
- The `AbstractMonad` `CalibrationProblem` constructor takes its reference variation *from the reference* and offers no keyword override, matching `createTrial(method, reference::AbstractMonad, avs; ...)`. (Note `run(::GSAMethod, reference::AbstractMonad, avs; ...)` does accept one, but by accident of keyword precedence — the user's value lands rightmost in the `kwargs...` splat and wins — rather than by design.)
