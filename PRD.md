...
- **Discrete and mixed parameter spaces are calibratable.** A `DiscreteVariation` or `CoVariation{<:DiscreteVariation}` may be passed alongside continuous parameters. It becomes a `DiscreteUniform` over its value indices, so an ABC-SMC particle coordinate is a CDF value in [0, 1] exactly as for a continuous parameter and the quantile does the quantising. The four perturbation kernels therefore need no discrete counterpart: they operate on CDF coordinates and never see a target value, and `_minDiagVar` already floors a variance that collapses when a generation's particles land in one bin. The posterior CSVs and the `:posterior`/`:transition` recipes report the *level*, not the index.
  ...
  - `generation_{NNN}` parameter TOML entries for a discrete source record `"values"` — the levels themselves — rather than the internal `DiscreteUniform`, since the levels are what a reader needs to know which values the run could have visited.
...
- Per-generation results are saved in two forms. **Human-readable:** `generations/generation_{NNN}.csv` (columns: user-friendly parameter display names, `weight`, `distance`, `monad_id`) and `generations/generation_{NNN}.toml` (generation-level metadata: `max_epsilon_accepted`, `epsilon_threshold` when the generation had one, `acceptance_rate`, `ess`, `n_evaluations`). **Machine-readable (resume):** `generation_cdfs/generation_{NNN}.csv` (raw CDF coordinates + `weight`, `distance`, `monad_id`). `NNN` is zero-padded to `ndigits(max_nr_populations)`.
...
- `GenerationResult` stores `acceptance_rate` (all proposals passing epsilon / all proposals evaluated — **not** capped at `population_size`; overshoot in the final batch does not bias the rate downward), `ess` (= 1/Σwᵢ²), `n_evaluations`, `max_epsilon_accepted` and `epsilon_threshold` per generation. These are logged after each generation and saved to `generations/generation_{NNN}.toml`.
