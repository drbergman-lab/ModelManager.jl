...
- [x] Failed particle evaluation — `on_evaluation_failure` keyword on `runABC`/`runCalibration`/`resumeABC` (`:reject` default, `:error`); a particle whose `summary_statistic`/`distance` raises (typically because every simulation in its monad failed and the emptied monad was deleted) gets distance `Inf` and is rejected, with a throttled warning (5 per generation + a per-generation total) that names the monad, its failed simulations' output folders, and whether the monad still exists; generation 1 drops non-finite distances before setting ε and errors only if no particle survives; rejected monads are never added to the `SimulationBank`; low-success batches warn despite `quiet=true`
