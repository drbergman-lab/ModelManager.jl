...
- [x] HPC submission robustness — a submission `sbatch` refuses is retried while the message looks transient (submit limits, controller timeouts) and otherwise stops the run, leaving the simulation at `Not Started`; `run` closes its queue on any exit so nothing is left at `Queued`; `defaultJobOptions` requests a job name and, through the backend's `simulationThreads`, the CPUs a simulation will use; `Function`-valued job options receive the `Simulation`
