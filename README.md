...
- [x] Schema migrations — `up.jl` framework with `upgradePackage`, `upgradeToMilestone`; migrations target the version loaded in the session rather than the one installed in the environment, read from the package defining the simulator type, since the milestone list comes from that loaded code — so updating the environment mid-session defers the schema change to the next session instead of recording a version whose migration never ran, and `upgradePackage` refuses a target beyond the loaded version for direct callers
