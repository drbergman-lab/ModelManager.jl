...
- **Concurrent trial creation is unsupported** — in one session or across sessions. Two Julia sessions cannot corrupt the SQLite file (SQLite serializes writers), but `Sampling(monads, inputs)` and `_findOrCreateTrialID(samplings)` in `src/classes.jl` scan for a matching row before inserting, with no `UNIQUE` constraint to fall back on, so each could insert a duplicate. They also race on the constituent-ID CSVs, which no database lock covers. (`_findOrCreateTrialID` is reached only through the `Trial(Ss)` constructor; the exported `trialID(samplings)` is a pure lookup and never inserts.)
  ...
  The coalesced calibration views (`Sampling(calibration)`, `Sampling(calibration, generation)` in `src/calibration/calibration.jl`) call the same `Sampling` find-or-insert, so they widen this surface rather than sidestepping it: two sessions asking for the same view concurrently could each insert a row for it. This is why the calibration *accessors* (`monadIDs`/`simulationIDs` on a `Calibration`) are pure reads that insert nothing — only the `Sampling` constructors materialize.
...
- ~~Wire the `post_processor` QoI builders into sensitivity analysis and calibration workflows.~~ **Done** — `src/qoi.jl` provides `QoI` plus `sensitivityFunction`/`summaryStatistic`/`postProcessor`. Note the premise was partly wrong: `populationCountQoI` never existed in this repo (the builders are PCMM-side), so the work was to build the MM seam rather than to rewire existing builders. A PCMM builder now becomes a `QoI` and feeds all three consumers.
- **Add `AUTOINCREMENT` to the primary keys.** Every MM table declares `<class>_id INTEGER PRIMARY KEY` with no `AUTOINCREMENT`, so SQLite assigns `max(rowid)+1` and hands a deleted object's ID to the next insert whenever the deleted row held the maximum (verified: insert 1–3, `DELETE` 3, insert → the new row *is* id 3, and its simulations get ids 1 and 2 back too). `AUTOINCREMENT` makes SQLite track a high-water mark in `sqlite_sequence` and never reuse. Cost is a schema migration, and `upgradeMilestones`/`upgradeToMilestone` are `AbstractSimulator` methods, so the milestone must be implemented by every downstream simulator — which is why this is worth folding into the next migration rather than doing on its own.
  ...
  **Most durable references already self-heal, so the remaining exposure is small.** Tag rows are deleted at all five choke points (`deleteSimulations`, `deleteMonad`, `deleteSampling`, `deleteTrial`, `deleteCalibration`), so the `tags` store never carries a stale reference — which makes tag-based recovery, the way the manual already tells users to hold onto work, structurally immune to reuse. Parent constituent CSVs are filtered and rewritten on deletion too. Two holes remain:

  - `deleteSimulations(ids; delete_supers=false)` returns *before* the parent-CSV filtering, so the monad keeps the deleted simulation's ID in `monads/{id}/simulations.csv`.
  - calibration's `generations/generation_{NNN}_monads.csv` is written before each batch for crash safety and no deletion path ever rewrites it.

  **The calibration case cannot corrupt a result, and the reason is structural.** A monad is deleted only when *every* one of its simulations failed, and such a monad has distance `missing`, which generation 1 drops and later generations reject — so a deleted monad is never an accepted particle, and the accepted-particle records (`generation_{NNN}.csv`, `generation_cdfs/`) never contain an alias-prone ID. `posterior`, `ConvergenceSummary` and `resumeABC` read only those. The blast radius is the monads record: `calibrationMonadIDs`, `monadIDs`/`simulationIDs`/`Sampling` on a `Calibration`, and `_lazyLoadRejectedFromDisk` for the `:transition` plot. Ranked by likelihood: reuse by the **next batch in the same generation** is the common case and is *benign* (the new monad genuinely belongs to that generation); reuse in a **later generation** mis-attributes one monad to the earlier generation's view while the run-wide view stays correct; and a hole that **survives the run** — which needs the total failure in the final batch, since otherwise the next batch fills it — lets a later unrelated monad join the run-wide view, silently if its inputs match and as a thrown same-inputs assertion if they do not.

  Aliases are diagnosable after the fact: the reused row is a new row, so `applyCreationTags` stamps it, and its `mm:created` postdates the generation that recorded it. A calibration-local mitigation short of the migration is to persist the monads the runner actually deleted — `_batchOutcome` already computes them as `without_success`, but only `failed_monads` (≥ 1 failed simulation, a different and larger set) is written — and subtract them per generation; that is exact for the later-generation and post-run cases and over-corrects by one monad in the same-generation case. Surfaced reviewing PR #32.
