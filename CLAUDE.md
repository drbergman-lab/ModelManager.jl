...
**Queued work has pre-written design briefs in [planning/](planning/) — read [planning/README.md](planning/README.md)
first for the execution order, the parallel-safe groups, and the cross-brief dependencies.** The items below
are not covered by a brief.

- ModelManager needs an upgrade path of its own, parallel to the downstream simulator package's. Today only the simulator package (e.g. PCMM) can migrate: `getDBPackageVersion` (`src/package_version.jl`) records *its* version via `dbVersionTableName`, and `upgradeMilestones`/`upgradeToMilestone` are `AbstractSimulator` hooks the backend implements. ModelManager's own version is never written to the database, so MM-owned state — its schema additions, the `data/outputs/calibrations/` layout, generation TOML keys — has no migration channel, and a ModelManager release that changes any of them can only be handled by ad-hoc read-fallbacks (as `planning/06-distance-distribution-plot.md` does for the epsilon rename). Needed: an MM version row plus an MM-owned milestone list, run alongside the simulator's, with a defined ordering between the two when both are pending.
