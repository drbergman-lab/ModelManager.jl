...
- **Primary keys are reusable rowids, so a deleted ID can be handed to a later object.** Every MM table declares `<class>_id INTEGER PRIMARY KEY` with no `AUTOINCREMENT`, and SQLite reuses the largest freed rowid (verified: insert 1–3, `DELETE` 3, insert again → the new row *is* id 3). Every durable cross-reference in the package is one of those IDs stored in a CSV — `monads/{id}/simulations.csv`, `samplings/{id}/monads.csv`, `trials/{id}/samplings.csv`, and calibration's `generations/generation_{NNN}_monads.csv` — so deleting an object and creating another can silently alias a stale reference onto an unrelated object. Adding `AUTOINCREMENT` to the primary keys fixes it everywhere at once (SQLite then never reuses, tracking the high-water mark in `sqlite_sequence`), at the cost of a schema migration — and `upgradeMilestones`/`upgradeToMilestone` are `AbstractSimulator` methods, so a milestone must be implemented by every downstream simulator. Until then the exposure is narrow but real: it needs a deletion followed by a creation, and for calibration views specifically a mismatched alias trips the `Sampling` inner constructor's "all monads must have the same inputs" assertion, while a same-inputs alias joins the view silently. A calibration-local mitigation is to record the monads the runner actually deleted — `_batchOutcome` already computes them as `without_success`, but only `failed_monads` (≥ 1 failed simulation, which is a different and larger set) is written to disk. Surfaced reviewing PR #32.
