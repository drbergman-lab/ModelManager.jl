...
To simulate at the drawn parameter sets, hand the frame to [`createTrial`](@ref), which builds one
monad per distinct parameter set against the run's reference variation, exactly as the calibration
built its own, and returns a [`Sampling`](@ref) for [`run`](@ref). A plain draw resolves to the
monad it came from, so nothing new is simulated unless you raise `n_replicates`; a smoothed draw
gets a fresh monad. This is the posterior predictive check at new parameter values:

```julia
sampling = createTrial(result, points)   # or createTrial(Calibration(42), points)
run(sampling)
